...
- [~] HIGH | Report-0256 | 5 deal-child tables have NO FK at all → silent orphan rows on `DELETE FROM deals` (deal_sim_inputs, deal_owner_history, deal_health_history, deal_deadlines, deal_stars). Needs one-time orphan-purge + migration to add `ON DELETE CASCADE`. | rcm_mc/deals/* | MR1057 | 91097a1 | 2026-04-26 (DDL updated for fresh DBs — verified cascade on PRAGMA foreign_key_list + behavioural smoke; live-DB ALTER migration still pending — see MR1059)
- [ ] MEDIUM | Report-0256 | Live-DB ALTER migration follow-up to MR1057 — existing deployments still lack the FK on the 5 tables; `CREATE TABLE IF NOT EXISTS` is a no-op so live DBs need a one-shot orphan-purge + table-rebuild migration. | rcm_mc/deals/* | MR1059
