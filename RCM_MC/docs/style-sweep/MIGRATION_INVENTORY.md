...
**Status (as of batch 38, 2026-05-28)**: 38 batches shipped/in-flight; ~134 routes effectively swept; **Groups B and D both fully cleared**; Group C trope sweep at 59 surfaces cleaned + **7 gradients retired** + 3 box-shadows removed.
...
**Wave-9 (batch 38) shipped** — 4 more decorative 135° gradients retired (semantic severity borders preserved):
- `ic_memo_page.py` (`.ic-memo-bar-total` 135° → flat brand tint; semantic brand-deep left-stripe preserved)
- `bridge_audit_page.py` (`.ba-counter-card` 135° → flat panel; semantic positive border preserved; radius capped)
- `bear_case_page.py` (`.bc-callout` default decorative stripe stripped; `.bc-verdict-card` 135° → flat; semantic negative border preserved; radius capped)
- `analysis_workbench.py` (`.ov-banner` 135° → flat brand tint; decorative left stripe stripped)
...
- `data_public/cms_sources_page.py` (code-block 3px brand stripe)
- `models_page.py` (2 surfaces: DCF interpretation + Financials interpretation)
- `market_analysis_page.py` (Market analysis interpretation)
- `waterfall_page.py` (Waterfall interpretation)
- `market_intel_page.py` (Geographic Intelligence overlay panel)
