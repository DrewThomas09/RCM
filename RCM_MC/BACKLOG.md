...
| 2 | ✅ DONE — **P2 CIM Cross-Check vertical slice** — claim entry (market size $, provider count, margin %, payer mix %, volume) → independent HCRIS/universe estimates → variance table w/ green≤10/yellow≤25/red flags + drill-to-source + memo export. Hospital subsector, one state. | 43 | Fabricate claims from real TX data ±7/18/40% → exactly green/yellow/red fire; every estimate row links source; memo exports; pytest unit tests on variance math; screenshot · verified: tests/test_cim_crosscheck.py (30 passed); route /diligence/cim-crosscheck (server.py:4529) |
| 3 | ✅ DONE — **P7 Roll-Up Scenario Builder slice** — pick 2–3 real HCRIS CCNs in one state → combined volumes, NPR, payer blend, state share + HHI before/after, antitrust note >Δ200 | 40 | Hand-check aggregate math vs the 3 single-facility rows; HHI delta recomputed by hand; screenshot; tests on the combine math · verified: tests/test_rollup_scenario.py (13 passed); route /pipeline/rollup (server.py:8427) |
| 4 | ✅ DONE — **P4 percentile chip** — reusable ck_peer_percentile(value, dist) "p78 vs TX hospitals (n=412)" + sparkline; wire into deal quick-view + CIM estimates | 36 | Hand-check 3 facilities' percentile placement vs raw distribution; tests on percentile math incl. ties/NaN · verified: tests/test_peer_percentile.py (10 passed); ck_peer_percentile at _chartis_kit.py:2101 wired in deal_quick_view.py:105 |
| 5 | ✅ DONE — **P11 Data Quality dashboard** — one screen: registry sources × vintage, rows, null-rate on key fields, gap census (reuse gap_fill_registry), pages-consuming | 34 | Numbers match `rcm-mc data gaps` + 2 independent spot-checks; screenshot · verified: tests/test_data_quality_page.py + tests/test_data_quality_gap_chart.py (15 passed); route /data-quality (server.py:4662) |
| 6 | ✅ DONE — **P8b facility→rule exposure** — join provider type → applicable rules from regulatory_calendar; exposure panel on X-Ray with rule dates + source links | 33 | One hospital + one SNF-ish CCN map to correct rule sets; every headline cites its rule URL; tests on the join · verified: tests/test_regulatory_exposure.py (7 passed); panel wired in hcris_xray_page.py:769, route /diligence/hcris-xray (server.py:4526) |
| 7 | ✅ DONE — **P13 insight-bullet primitive** — template+significance-guard engine (suppress <0.5pp deltas); apply to /portfolio + state market page; copy-to-clipboard | 31 | 10 bullets read against panels: zero unsupported claims; guard test (tiny delta → no bullet) · verified: tests/test_insight_bullets.py (7 passed); ck_insight_bullets at _chartis_kit.py:11441 wired in portfolio_overview.py:373 + market_data_page.py:740 |
| 8 | ✅ DONE — **P1 deal switcher** — active-deal context in nav (cookie/session), screener/X-Ray/market pre-scope to deal geography; deal home links back | 31 | Create real-CCN deal, confirm 3 modules open pre-scoped (4 confirmed: CIM/roll-up/screener/X-Ray); tests on context carry · verified: tests/test_deal_context.py + tests/test_deal_context_prefill.py (15 passed); route /deal-context (server.py:4618) |
| 9 | ✅ DONE — **B in-UI model card** — holdout coverage + calibration summary for ridge+conformal on /methodology + predictive screener footer ("90% conformal band covered 89.3% on 2024 holdout") | 30 | Numbers reproduced by a checked-in script; never claims AI/LLM; test asserts wording · verified: tests/test_model_card.py + tests/test_model_card_no_ai.py (added, no-AI wording assertion) + ModelCardFooterTests (12 passed); route /methodology/calculations (server.py:4603), reproduced by scripts/eval_margin_model.py |
...
| 15 | ✅ DONE — **A empty-state sweep** — top-10 pages with ?state=ZZ / empty db: consistent ck_empty_state, no dead controls | 20 | Walker variant with empty db; screenshots · verified: tests/test_empty_state_sweep.py (3 passed, permanent gate over empty-db + ?state=ZZ variants via scripts/route_walker.py) |
