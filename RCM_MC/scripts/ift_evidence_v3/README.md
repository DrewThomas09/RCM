...
`RCM_MC/deliverables/IFT_Sourced_Evidence_Master_v3_0.xlsx` (245+ tabs, ~5,000
printed pages, 190+ native charts, facts F001–F430+, sources S001–S304+) from:
...
1. **The v2.7 master** (`rcm_mc/market_reports/reference/IFT_Sourced_Evidence_Master_v2_7.xlsx`)
   ...
   re-created natively from their parsed XML.
...
3. **Live government-API pulls** (CMS data.cms.gov, Care Compare PDC, BLS QCEW)
   — cached under `rcm_mc/market_reports/reference/ift_v3_cache/` (gzipped
   JSON) with a manifest (endpoint, dataset UUID, filters, rows, SHA-256,
   UTC timestamp) rendered on the workbook's Pull_Manifest tab.
...
python3 pull.py && python3 pull2.py && python3 pull3.py && python3 pull4.py && python3 pull5.py
...
export IFT_V3_OUT=../../deliverables/IFT_Sourced_Evidence_Master_v3_0.xlsx
python3 assemble.py
...
- **V1 fidelity** — every carried v2.7 cell recalculates to the same value
  (10,679 cells, 0 diffs), excluding the rebuilt README/Source_Index and the
  edits enumerated on V3_Change_Log.
- **V2 zero errors** — LibreOffice full recalc of all 45k+ formulas yields no
  Excel error cells.
...
`assemble.py`. Tests: `RCM_MC/tests/test_ift_evidence_v3.py` validates the
committed deliverable without regenerating it.
