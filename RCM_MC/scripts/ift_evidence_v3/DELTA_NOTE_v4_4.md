...
## Run 8: the national brand-name invisibility test - why GMR and Priority read as near-zero in NPPES, and why fleet (not licensed EMTs) is the scale unit
...
Holdings). Run 8 takes that same undercount and tests it at the **national brand
level** against the live NPPES API, then answers the sizing question it forces:
if you cannot count the operator, what do you count? Appended F637-F645,
sources S991-S997, finding 131; no shipped ID renumbered.
...
|---|---|---:|---|
| F637 | `Global Medical Response` | **0** | The parent has no operating NPI - it is a holding company and bills only through subsidiaries. |
| F638 | `Rural/Metro` and `Rural Metro` | **0** | A major GMR ground brand - nothing under its own name. |
| F639 | `American Medical Response` | **65** | Fragmented across ~15 distinct legal names (AMR *of Connecticut / Massachusetts / Texas / Southern California / West / Mid-Atlantic / Northwest / Tennessee* …) **plus** local-brand DBAs that only matched because "American Medical Response" is a *former/other* name on those records (Randle Eastern, LifeFleet Southeast, Mercy Inc, Metro Ambulance, ParaMed, Hank's Acquisition Corp). |
| F640 | `Priority Ambulance` | **3** | All three are **"Shoals Ambulance LLC"** - one Priority regional brand. The other 21 brands do not carry the Priority name. |
...
- **F641 - GMR (parent, incl. AMR):** "more than 7,000 ground ambulances and
  support vehicles," 382 air bases, "nearly six million patient encounters" last
  year (GMR fleet-expansion release, S992).
- **F642 - AMR (GMR's 911/IFT ground arm):** ~34,000 employees, "more than 27,000"
  paramedics/EMTs/RNs and other professionals, "more than 4.4 million" transports/yr,
  40 states + DC (amr.net / AMR profile, S993-S994).
- **F643 - Priority Ambulance:** ~400 emergency and non-emergency vehicles, 4,300
  ...
  **22 brands across 15 states** - and Priority states it *deliberately preserves*
  each acquired company's name (priorityambulance.com/about, S995).
...
vehicles was carried but **deliberately never divided**). Run 8 answers *which
denominator recovers scale* using the two national anchors, and quarantines the
ratios themselves as MODELED:

- **F644 (DERIVED / MODELED -> Excluded_Not_Sourced):** transports per **vehicle**/yr
...
- **F645 (DERIVED / MODELED -> Excluded_Not_Sourced):** transports per **licensed
...
sharing the word "Priority"; Run 8's F640/F643 are the Knoxville national roll-up.

## The firewall, held exactly

The NPPES counts (F637-F640) are a **primary pull we ran ourselves** - tier A,
OBSERVED, reproducible by re-running the four name queries against the public NPPES
API. The operator scale figures (F641-F643) are **company self-reports** - PUBLIC-
WEB / CLAIMED (tier C), carried to size the gap, never quoted as market
measurement. The two ratios (F644-F645) are **MODELED** and sit on
...
unchanged. NEMSIS's 34.2M all-EMS-events figure (S996) is carried only as
...
- ID ledgers contiguous: F637-F645 appended after F623; S991-S997 after S447;
  finding 131 after 117. No shipped ID renumbered.
