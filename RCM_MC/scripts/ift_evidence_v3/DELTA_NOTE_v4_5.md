# IFT Sourced Evidence Master v4.5 - delta note

## Run 9: the four player tiers, each with its own public-data fingerprint - the guidance that tells you which asset actually sees an operator

Run 8 answered *how to size* (fleet, not licensed EMTs) and *why the national
brands are invisible* in NPPES. Run 9 turns that into an operator-tier map: the
...
not just prose. Appended F646-F652, sources S998-S1000, finding 132; no shipped ID
renumbered.
...
|---|---|---:|---|
| F646 | `Global Medical Response*` | **0** | Wildcard-confirmed: the national parent has no NPI. `Priority Ambulance*` → **8**, but only **3** (Shoals Ambulance LLC) are the real Knoxville roll-up; the other 5 are unrelated "Priority Ambulance" **name-collisions**. So the national name-search is both under-representative *and* polluted. (Refines F637/F640.) |
| F647 | `Acadian Ambulance*` | **12** | Scaled-regional fingerprint: **findable** - parent + clean state-suffixed LLCs (LA/TX/MS/TN/AL), air arm included. |
| F648 | `Falck*` | **17** | Scaled-regional with noise: ~10 regional ambulance corps (Falck [region] Corp) **plus** same-name non-ambulance companies (Falck Eye Center, Falck Therapy) - filter by taxonomy 3416*, not name. |
...
- **F649 - Acadian Ambulance (scaled regional):** 750 ground ambulances, ~800,000
  transports/yr, 4 states (LA/TX/MS/TN), 8 helicopters + 5 fixed-wing, ~$500M-$734M
  revenue (Wikipedia / acadian.com, S999). Note: Acadian's 800K transports *exceed*
  Priority's 610K - "national" is a roll-up/contracting structure, not necessarily
  the largest volume.
- **F650 - DocGo (NASDAQ: DCGO, scaled regional):** FY2024 total revenue $616.6M;
  Transportation Services segment ~$190M/yr (emergency + non-emergency + wheelchair)
  (DocGo FY2024 10-K/8-K, S1000, tier B - a filer's own audited figure).

## Fleet_vs_Labor + Tier_Guidance (Panel C - DERIVED framework)

- **F651 (DERIVED / MODELED):** the four-tier band table (transports / fleet /
  revenue / NPIs-per-operator) encoded in `ift_player_tiers._TIERS`. Bands are wide
  and indicative, quarantined until the measured panel (Run 8 human action item)
  replaces them.
- **F652 (DERIVED):** the per-tier **public-data recipe** - National: fleet +
...
**Finding 132:** IFT operator tiers have distinct public-data fingerprints, and
...
the bottom.** Extends Finding #46 and Finding 131 (Run 8).

## The firewall, held exactly

The tier fingerprints (F646-F648) are a **primary NPPES pull we ran** - OBSERVED,
tier A, reproducible. The operator scale figures (F649-F650) are company
self-reports / a filer's own numbers - CLAIMED / tier B, carried to set bands,
never quoted as the market. The bands and the recipe (F651-F652) are **DERIVED**
...
- `tests/test_ift_player_tiers.py` (5 tests) green: four tiers in rank order, every
  tier carries all guidance fields, the OBSERVED NPPES probe matches Run 8
  (GMR* 0, Acadian* 12), `source_label` names OBSERVED/CLAIMED/DERIVED.
- ID ledgers contiguous: F646-F652 after F645; S998-S1000 after S997; finding 132
  after 118. No shipped ID renumbered.

## Human action items
1. The Run 8 measured panel now doubles as the tier-boundary calibrator: pull CMS
