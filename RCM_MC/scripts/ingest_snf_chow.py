"""Snapshot CMS SNF Change of Ownership → consolidation/transaction signal.

Source: CMS "Skilled Nursing Facility Change of Ownership" (data.cms.gov,
public). BUILD-TIME: fetch the CHOW file and aggregate to nursing-home
...
Honesty: CHOW counts are Medicare-enrolled SNF ownership CHANGES — a
...
_OUT = Path(__file__).resolve().parent.parent / "rcm_mc" / "data" / "vendor" / "snf_chow"
_URL = ("https://data.cms.gov/sites/default/files/2026-04/"
        "75f322b9-c582-42a1-b4e0-4f30ed373f4a/SNF_CHOW_2026.04.01.csv")
...
def main(argv=None) -> int:
    ...
    by_state_year.to_csv(_OUT / "snf_chow_state_year.csv", index=False)

    by_year = (df.groupby("year").size().reset_index(name="chow_count")
               .sort_values("year"))
    by_year.to_csv(_OUT / "snf_chow_national_year.csv", index=False)

    yrs = sorted(df["year"].unique())
    (_OUT / "snf_chow_report.json").write_text(json.dumps({
        "snapshot_date": datetime.now(timezone.utc).date().isoformat(),
        "extract": "SNF_CHOW_2026.04.01",
        "total_chows": int(len(df)),
        "states": int(df["state"].nunique()),
        "year_min": int(yrs[0]) if yrs else None,
        "year_max": int(yrs[-1]) if yrs else None,
        "source": "CMS SNF Change of Ownership (public); identifiers dropped",
    }, indent=2))
