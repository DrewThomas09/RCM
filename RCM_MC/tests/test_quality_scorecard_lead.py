"""The /quality-scorecard page leads with the computed quality value.

The page used to open with an 8-up KPI strip and several charts/tables,
leaving the headline (overall quality score + EV uplift from quality)
split across KPIs #1/#8 and the bottom thesis. This pins that a
ck_value_anchor band now surfaces it at the top.
"""
from __future__ import annotations

import unittest

from rcm_mc.ui.data_public.quality_scorecard_page import (
    render_quality_scorecard,
)


class QualityScorecardLeadAnchorTests(unittest.TestCase):
    def _html(self) -> str:
        return render_quality_scorecard({})

    def test_lead_value_anchor_present(self):
        html = self._html()
        self.assertIn("ck-value-anchor", html)
        self.assertIn("QUALITY VALUE", html)
        self.assertIn("EV uplift", html)

    def test_anchor_leads_before_charts_and_bottom_thesis(self):
        html = self._html()
        self.assertLess(
            html.index("ck-value-anchor"),
            html.index("Overall Quality Score"),
        )
        self.assertLess(
            html.index("ck-value-anchor"),
            html.index("Quality Thesis"),
        )


if __name__ == "__main__":
    unittest.main()
