...
class TestExtendedSeed26(unittest.TestCase):
    """Tests for extended_seed_26.py (deals 536-555)."""
    ...
    def test_seed_26_unique_ids(self):
        from rcm_mc.data_public.extended_seed_26 import EXTENDED_SEED_DEALS_26
        ids = [d["source_id"] for d in EXTENDED_SEED_DEALS_26]
        self.assertEqual(len(ids), len(set(ids)))

    def test_seed_536_aac_disaster(self):
        """American Addiction Centers — over-leveraged behavioral health disaster."""
        deal = self.corpus.get("seed_536")
        self.assertIsNotNone(deal)
        self.assertLess(deal["realized_moic"], 1.0)

    def test_seed_538_oak_street_homerun(self):
        """Oak Street Health / General Atlantic — VBC primary care 2.8x."""
        deal = self.corpus.get("seed_538")
        self.assertIsNotNone(deal)
        self.assertGreaterEqual(deal["realized_moic"], 2.5)

    def test_seed_555_present(self):
        deal = self.corpus.get("seed_555")
        self.assertIsNotNone(deal)

    def test_seed_540_compassus_hospice(self):
        deal = self.corpus.get("seed_540")
        self.assertIsNotNone(deal)
        self.assertIn("Compassus", deal["deal_name"])


class TestExtendedSeed25(unittest.TestCase):
    """Tests for extended_seed_25.py (deals 516-535)."""
    ...
    def test_seed_25_unique_ids(self):
        from rcm_mc.data_public.extended_seed_25 import EXTENDED_SEED_DEALS_25
        ids = [d["source_id"] for d in EXTENDED_SEED_DEALS_25]
        self.assertEqual(len(ids), len(set(ids)))

    def test_seed_519_cotiviti_homerun(self):
        """Cotiviti/Veritas — payment integrity SaaS 3.5x."""
        deal = self.corpus.get("seed_519")
        self.assertIsNotNone(deal)
        self.assertGreaterEqual(deal["realized_moic"], 3.0)

    def test_seed_522_akumin_disaster(self):
        """Akumin radiology — over-levered rollup disaster."""
        deal = self.corpus.get("seed_522")
        self.assertIsNotNone(deal)
        self.assertLess(deal["realized_moic"], 1.0)
...
class TestExtendedSeed24(unittest.TestCase):
    """Tests for extended_seed_24.py (deals 496-515)."""
    ...
    def test_seed_24_unique_ids(self):
        from rcm_mc.data_public.extended_seed_24 import EXTENDED_SEED_DEALS_24
        ids = [d["source_id"] for d in EXTENDED_SEED_DEALS_24]
        self.assertEqual(len(ids), len(set(ids)))

    def test_seed_501_shields_homerun(self):
        """Shields Health Solutions — specialty pharmacy 5x."""
        deal = self.corpus.get("seed_501")
        self.assertIsNotNone(deal)
        self.assertGreaterEqual(deal["realized_moic"], 4.5)

    def test_seed_498_ati_disaster(self):
        """ATI Physical Therapy SPAC — disaster below 1x."""
        deal = self.corpus.get("seed_498")
        self.assertIsNotNone(deal)
        self.assertLess(deal["realized_moic"], 1.0)

    def test_seed_510_premise_health_homerun(self):
        """Premise Health / Warburg — employer health 5.5x."""
        deal = self.corpus.get("seed_510")
        self.assertIsNotNone(deal)
        self.assertGreaterEqual(deal["realized_moic"], 5.0)

    def test_seed_515_present(self):
        deal = self.corpus.get("seed_515")
        self.assertIsNotNone(deal)


class TestExtendedSeed23(unittest.TestCase):
    """Tests for extended_seed_23.py (deals 476-495)."""
    ...
    def test_seed_23_count(self):
        from rcm_mc.data_public.extended_seed_23 import EXTENDED_SEED_DEALS_23
        self.assertEqual(len(EXTENDED_SEED_DEALS_23), 20)

    def test_seed_23_unique_ids(self):
        from rcm_mc.data_public.extended_seed_23 import EXTENDED_SEED_DEALS_23
        ids = [d["source_id"] for d in EXTENDED_SEED_DEALS_23]
        self.assertEqual(len(ids), len(set(ids)))

    def test_seed_479_landmark_homerun(self):
        """Landmark Health/Optum — strong MOIC on VBC home visit model."""
        deal = self.corpus.get("seed_479")
        self.assertIsNotNone(deal)
        self.assertGreaterEqual(deal["realized_moic"], 4.0)

    def test_seed_482_gastro_health_homerun(self):
        """Gastro Health/Audax — GI platform homerun."""
        deal = self.corpus.get("seed_482")
        self.assertIsNotNone(deal)
        self.assertGreaterEqual(deal["realized_moic"], 4.0)

    def test_seed_492_summit_pacific_distressed(self):
        """Summit Pacific rural hospital — loss-making distressed exit."""
        deal = self.corpus.get("seed_492")
        self.assertIsNotNone(deal)
        self.assertLess(deal["realized_moic"], 1.0)

    def test_seed_495_present(self):
        deal = self.corpus.get("seed_495")
        self.assertIsNotNone(deal)


class TestExtendedSeed22(unittest.TestCase):
    """Tests for extended_seed_22.py (deals 456-475)."""
    ...
    def test_seed_22_unique_ids(self):
        from rcm_mc.data_public.extended_seed_22 import EXTENDED_SEED_DEALS_22
        ids = [d["source_id"] for d in EXTENDED_SEED_DEALS_22]
        self.assertEqual(len(ids), len(set(ids)))
    ...
    def test_seed_460_aspen_dental_homerun(self):
        """Aspen Dental / LGP — dental DSO homerun."""
        deal = self.corpus.get("seed_460")
        self.assertIsNotNone(deal)
        self.assertGreaterEqual(deal["realized_moic"], 3.5)

    def test_seed_467_ardent_health_present(self):
        deal = self.corpus.get("seed_467")
        self.assertIsNotNone(deal)
    ...
    def test_seed_471_webmd_high_irr(self):
        """WebMD take-private — 3-year hold, strong IRR."""
        deal = self.corpus.get("seed_471")
        self.assertIsNotNone(deal)
        self.assertGreaterEqual(deal["realized_irr"], 0.40)
...
class TestExtendedSeed28(unittest.TestCase):
    """Tests for extended_seed_28.py (deals 576-595)."""
    ...
    def test_seed_28_unique_ids(self):
        from rcm_mc.data_public.extended_seed_28 import EXTENDED_SEED_DEALS_28
        ids = [d["source_id"] for d in EXTENDED_SEED_DEALS_28]
        self.assertEqual(len(ids), len(set(ids)))

    def test_seed_578_northstar_anesthesia_kkr(self):
        """NorthStar Anesthesia / KKR — major anesthesia rollup."""
        deal = self.corpus.get("seed_578")
        self.assertIsNotNone(deal)
        self.assertGreaterEqual(deal["ev_mm"], 2_000.0)
