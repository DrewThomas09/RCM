...
- **v5 editorial UI** (May 2026) — every partner-facing page now renders through `chartis_shell` in `rcm_mc/ui/_chartis_kit.py`. Editorial palette (navy / teal / parchment), Source Serif 4 + Inter Tight, top-nav with six sections (Home / Pipeline / Diligence / Library / Research / Portfolio), sub-nav rails, auto-breadcrumbs, Cmd+K command palette over 69 surfaces, toast/flash on state-changing POSTs, opt-in tutorial intros. Editorial primitives: `ck_page_title`, `ck_kpi_block`, `ck_empty_state`, `ck_provenance_tooltip`, `render_insights_page`. See [`rcm_mc/ui/README.md`](rcm_mc/ui/README.md) for the full kit catalog
