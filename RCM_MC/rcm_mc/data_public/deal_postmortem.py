...
def _build_attribution() -> List[LeverAttribution]:
    return [
        ...
        LeverAttribution("Payer Rate Uplift", 2.8, 1.2, 0.43,
                         "BCBS renewal at +3.0% (plan +5.0%)", "UHC renegotiation yielded 0.0% rate lift"),
        LeverAttribution("Multiple Arbitrage", 15.5, 6.2, 0.40,
                         "Buyer universe validated", "Exit multiple compressed to 13.20x vs 14.50x plan"),
...
def _build_lessons() -> List[LessonLearned]:
    return [
        ...
        LessonLearned("Technology", "EHR unification routinely slips 6+ months post-close",
                      "Extend PMI timeline budget + $2.00M contingency", "high"),
        LessonLearned("Labor Market", "Locum MD costs rose 40.0% in COVID era",
        ...
        LessonLearned("Payer Negotiation", "Large payer renewals historically deliver 30-50% of plan",
                      "Underwrite payer uplift at 60.0% of negotiated target", "high"),
        ...
        LessonLearned("Integration Investment", "Integration spend exceeded budget by 22.0%",
                      "Add 20.0% contingency to integration budget", "high"),
...
def _build_value_bridge() -> List[ValueBridge]:
    return [
        ...
        ValueBridge("Multiple Arbitrage (13.00x → 14.50x plan)", 52.5, 0.0, -52.5, -1.0),
        ValueBridge("Multiple Compression (Actual 13.00x → 13.20x)", 0.0, 22.8, 22.8, float('inf')),
...
def _build_counterfactuals() -> List[CounterfactualScenario]:
    return [
        ...
        CounterfactualScenario("Exit at 14.50x (on plan)", "Held 6 more months into 2026 upcycle", 0.32, "speculative"),
