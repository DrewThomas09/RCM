...
def _landing(qs: Optional[Dict[str, List[str]]] = None) -> str:
    # Pre-fill from query params (e.g. handoff from HCRIS X-Ray) when
    # present; else Steward-replay defaults.
    qs = qs or {}

    def _qs_or(key: str, default: str) -> str:
        vals = qs.get(key) or []
...
def _hero_stats(result: DealMCResult, scenario_name: str) -> str:
    ...
    # Plain-English summary keyed off P50 MOIC + P(sub 1x).
    p50 = result.moic_p50
    sub1 = result.prob_sub_1x
    if p50 >= 2.5 and sub1 <= 0.10:
        summary = (
            f"The distribution clears a 2.5x MOIC bar at the P50 with "
            f"{sub1*100:.1f}% downside probability. PE healthcare deals "
            f"underwriting to <3.0x and exiting at 2.5x+ are the top "
            f"quintile of realized outcomes. Execute the thesis."
        )
    elif p50 >= 1.8:
        summary = (
            f"P50 MOIC of {p50:.2f}x is in the acceptable but not-top-"
            f"quintile band. Downside probability of {sub1*100:.1f}% "
            f"means roughly 1-in-{max(1, int(1/max(sub1,0.01)))} scenarios "
            f"loses money. Stress-test the largest variance driver "
            f"before proceeding."
        )
    elif p50 >= 1.2:
        summary = (
            f"Marginal P50 MOIC ({p50:.2f}x) with non-trivial "
            f"{sub1*100:.1f}% loss probability. The deal clears the "
            f"cost of capital on average but has a real chance of "
            f"impairment. Consider offer-shape modifications from the "
            f"Counterfactual Advisor."
        )
    else:
        summary = (
            f"P50 MOIC ({p50:.2f}x) is below the typical 1.5x hurdle "
            f"rate. {sub1*100:.1f}% of trials lose capital. This is a "
            f"walkaway unless material offer-shape or structural "
...
def render_deal_mc_page(qs: Optional[Dict[str, List[str]]] = None) -> str:
    ...
    # Pipeline-hydration shortcut: ?from_pipeline=<fixture> runs the
    # full thesis pipeline against the fixture + optional metadata
    # and uses the pipeline's populated DealScenario instead of
    # hand-typed params.  Partners coming from the pipeline page
    # land in Deal MC with every driver already wired.
    from_pipeline = first("from_pipeline")
    hydrated_from_pipeline = False
    if from_pipeline:
        try:
            from ..diligence.thesis_pipeline import (
                PipelineInput, run_thesis_pipeline,
            )
            pipeline_inp = PipelineInput(
                dataset=from_pipeline,
                deal_name=first("deal_name") or from_pipeline,
                enterprise_value_usd=float_or("ev_usd", 0.0) or None,
                equity_check_usd=float_or("equity_usd", 0.0) or None,
                debt_usd=float_or("debt_usd", 0.0) or None,
                revenue_year0_usd=float_or("revenue_usd", 0.0) or None,
                ebitda_year0_usd=float_or("ebitda_usd", 0.0) or None,
                entry_multiple=float_or("entry_multiple", 0.0) or None,
                specialty=first("specialty") or None,
                landlord=first("landlord") or None,
                lease_term_years=int_or("lease_term_years", 0) or None,
                lease_escalator_pct=float_or("lease_escalator_pct", 0.0) or None,
                ebitdar_coverage=float_or("ebitdar_coverage", 0.0) or None,
                annual_rent_usd=float_or("annual_rent_usd", 0.0) or None,
                oon_revenue_share=float_or("oon_revenue_share", 0.0) or None,
                market_category=first("market_category") or None,
                ehr_vendor=first("ehr_vendor") or None,
                n_runs=int_or("n_runs", 1500),
            )
            report = run_thesis_pipeline(pipeline_inp)
            if report.deal_scenario is not None:
                scn = report.deal_scenario
                hydrated_from_pipeline = True
        except Exception:  # noqa: BLE001
            hydrated_from_pipeline = False

    if not hydrated_from_pipeline:
        if not first("ev_usd"):
            return _landing(qs)
        try:
            scn = DealScenario(
    ...
    if scn.equity_check_usd <= 0 or scn.revenue_year0_usd <= 0:
        return chartis_shell(
            f'<div style="padding:24px;color:{P["negative"]};">'
            f'<h2 style="font-size:20px;margin-bottom:8px;">'
            f'Cannot run Deal MC on this input.</h2>'
            f'<p style="color:{P["text_dim"]};line-height:1.6;'
            f'max-width:700px;">'
            f'<strong>Equity check</strong> and '
            f'<strong>Year-0 revenue</strong> must both be positive. '
            f'You supplied equity '
            f'<code>${scn.equity_check_usd/1e6:,.1f}M</code>, '
            f'revenue <code>${scn.revenue_year0_usd/1e6:,.1f}M</code>.'
            f'</p>'
    ...
    # HCRIS cross-links to distressed hospitals would land here.
    if scn.ebitda_year0_usd <= 0:
        return chartis_shell(
            ...
            f'<code>${scn.ebitda_year0_usd/1e6:,.1f}M</code>. '
            f'A valid LBO requires a positive base-case EBITDA so '
            f'the simulator can compute entry multiples, exit '
            f'proceeds, MOIC, and IRR. If the target is currently '
    ...
    revenue_fan = _chart_panel(
        ...
        how_to_read=(
            "X-axis is hold year; Y-axis is revenue in USD. The "
            "solid center line is the median trial; inner shaded "
            "band spans P25-P75; outer band P10-P90. Wider fans "
            "mean more uncertainty."
        ),
        note=(
            "Revenue compounds organic growth, provider attrition, "
            "and V28 compression on Medicare-exposed revenue."
        ),
    )
    ...
    moic_hist = _chart_panel(
        ...
        how_to_read=(
            "Height of each bar is the share of trials in that MOIC "
            "bucket. Mass left of 1.0x = money-losing scenarios; "
            "mass right of 3.0x = home-run scenarios. A bell-shape "
            "centered at 2.0x is a typical PE thesis."
        ),
        note=(
    ...
    tornado = _chart_panel(
        ...
        how_to_read=(
            "Each bar is the MOIC change when one driver is pushed "
            "to an adverse value while all others stay at base. "
            "Worst-case bars on the left show where structural "
            "offer-shape modifications would have the biggest "
            "thesis-protective impact."
        ),
    ...
    # Deep-link to Exit Timing with the current scenario pre-seeded.
    # Partners running Deal MC naturally ask "so when do we exit?"
    # Pull Y0 EBITDA + equity + debt + an inferred peer median
    # multiple from the scenario.
    import urllib.parse as _urllib
    exit_qs = _urllib.urlencode({
        "target_name": scenario_name,
        "equity_check_usd": int(scn.equity_check_usd),
        "debt_year0_usd": int(scn.debt_usd),
        "ebitda_year0_usd": int(scn.ebitda_year0_usd),
        "ebitda_growth": f"{scn.organic_growth_mean:.3f}",
        "peer_median_multiple": f"{scn.exit_multiple_mean:.1f}",
    })
    exit_cta = (
        ...
        f'gap:16px;flex-wrap:wrap;position:relative;overflow:hidden;">'
        ...
        f'margin-top:2px;">When + to whom should we exit?</div>'
        f'<div style="font-size:11px;color:{P["text_dim"]};margin-top:4px;'
        f'line-height:1.5;max-width:620px;">'
        f'Runs IRR × MOIC curves across candidate exit years 2-7 and '
        f'scores each buyer type (strategic / PE secondary / IPO / '
        f'sponsor hold) against your scenario. Current Deal MC '
        f'inputs are pre-seeded.'
        f'</div></div>'
        f'<a href="/diligence/exit-timing?{exit_qs}" '
        f'style="padding:10px 20px;background:{P["accent"]};'
        f'color:{P["panel"]};border:0;font-size:10px;letter-spacing:1.5px;'
        f'text-transform:uppercase;font-weight:700;text-decoration:none;'
        f'border-radius:3px;white-space:nowrap;">'
        f'Open Exit Timing →</a>'
        f'</div>'
    )
