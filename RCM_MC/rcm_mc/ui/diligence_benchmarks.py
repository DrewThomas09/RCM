...
def _placeholder_page() -> str:
    body = (
        ...
        f'margin-bottom:8px;">Step 2 — KPI Benchmarking & Stress Testing</div>'
        f'  <div style="font-size:13px;color:{P["text_dim"]};max-width:720px;'
        f'line-height:1.55;">Attach a Canonical Claims Dataset in '
        f'<a href="/diligence/ingest" style="color:{P["accent"]};">Step 1</a> '
...
def _denial_pareto(rows: Iterable[DenialStratRow]) -> str:
    ...
    return (
        ...
        f'style="color:{P["accent"]};">Step 3 — Root Cause</a>.'
        f'</div>'
        f'{"".join(items)}'
        f'</div>'
    )
...
def _cash_waterfall_section(report: Optional[CashWaterfallReport]) -> str:
    ...
    return (
        ...
        f'<a href="/diligence/root-cause" style="color:{P["accent"]};">Step 3</a>.'
        f'  </div>'
        f'  {topline}'
