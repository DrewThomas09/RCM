...
def render_focused_deal_bar(
    ...
    *,
    ...
    return (
        ...
        '<div class="ctx">'
        f'<span class="id" title="Deal id (slug used in URLs and exports)">'
        f'{_html.escape(deal_id)}</span>'
        f'<span class="name" title="Deal name">'
        f'{_html.escape(name)}</span>'
        f'<span class="meta" title="Lifecycle stage — sourced / IOI / LOI / SPA / closed / hold / exit">'
        f'STAGE <span class="v">{_html.escape(stage_label)}</span></span>'
        f'<span class="meta" title="Entry enterprise value (EV = Entry EBITDA × Entry Multiple)">'
        f'EV <span class="v">{number_maybe(ev, format="ev")}</span></span>'
        f'<span class="meta" title="Multiple on Invested Capital (Exit equity ÷ Entry equity)">'
        f'MOIC <span class="v">{number_maybe(moic, format="moic")}</span></span>'
        f'<span class="meta" title="Internal Rate of Return — annualized equity return">'
        f'IRR <span class="v">{number_maybe(irr, format="pct")}</span></span>'
        '</div>'
