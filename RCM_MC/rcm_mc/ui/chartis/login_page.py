...
def _render_signin_form(*, error: Optional[str], next_url: str) -> str:
    ...
    # Demo-credential pre-fill is a UX shortcut for the LOCAL
    # `python demo.py` workflow only. On a real deploy (pedesk.app
    # etc.) the seeded demo / partner users don't exist in the
    # production database, so pre-filling them with credentials that
    # 401 is misleading. Gate behind RCM_MC_DEMO_PREFILL=1 (set in
    # demo.py via env), and leave the form blank on production.
    import os as _os
    _prefill = _os.environ.get("RCM_MC_DEMO_PREFILL", "").strip() == "1"
    user_value = ' value="demo"' if _prefill else ""
    pass_value = ' value="DemoPass!1"' if _prefill else ""
    return (
        '<div id="signInForm">'
        f'{err_html}'
        ...
        f'placeholder="partner@firm.com" required{user_value} '
        'inputmode="email" autocomplete="username" autofocus/>'
        '</div>'
        '<div class="field">'
        '<label for="login-password">Password</label>'
        '<input type="password" id="login-password" name="password" '
        f'placeholder="••••••••••••" required{pass_value} '
