...
def _matters_table(items) -> str:
    ...
    for i, m in enumerate(items):
        ...
        cells = [
            f'{ck_data_cell(f"""{_html.escape(m.matter_id)}""", mono=True, weight=700)}',
            ...
            f'{ck_data_cell(f"""<span style="display:inline-block;padding:2px 8px;font-size:10px;font-family:JetBrains Mono,monospace;color:{s_c};border:1px solid {s_c};border-radius:2px;letter-spacing:0.06em">{_html.escape(m.stage)}</span>""", align="center")}',
...
def _types_table(items) -> str:
    ...
    for i, t in enumerate(items):
        rb = panel_alt if i % 2 == 0 else bg
        cells = [
            f'{ck_data_cell(f"""{_html.escape(t.matter_type)}""", mono=True, weight=700)}',
            f'{ck_data_cell(f"""{t.open_matters}""", align="right", mono=True, tone="acc")}',
            f'{ck_data_cell(f"""${t.alleged_amount_m:.1f}M""", align="right", mono=True, tone="neg", weight=700)}',
            ...
            f'{ck_data_cell(f"""{t.avg_time_open_months:.1f}""", align="right", mono=True, tone="dim")}',
            f'{ck_data_cell(f"""{t.win_rate_pct * 100:.1f}%""", align="right", mono=True, tone="pos")}',
        ]
        trs.append(f'<tr>{"".join(cells)}</tr>')
    return (f'<div class="ck-data-table-scroll"><table class="ck-data-table">'
            f'<thead><tr>{ths}</tr></thead><tbody>{"".join(trs)}</tbody></table></div>')
...
def _regulatory_table(items) -> str:
    ...
    for i, r in enumerate(items):
        ...
        cells = [
            f'{ck_data_cell(f"""{_html.escape(r.agency)}""", mono=True, weight=700)}',
            ...
            f'{ck_data_cell(f"""${r.estimated_fine_m:.1f}M""", align="right", mono=True, tone="neg", weight=700)}',
            f'<td style="text-align:left;padding:5px 10px;font-size:10px;color:{text_dim}">{_html.escape(r.resolution_pathway)}</td>',
            f'{ck_data_cell(f"""{_html.escape(r.timeline)}""", align="right", mono=True, tone="dim")}',
        ]
        trs.append(f'<tr>{"".join(cells)}</tr>')
    return (f'<div class="ck-data-table-scroll"><table class="ck-data-table">'
            f'<thead><tr>{ths}</tr></thead><tbody>{"".join(trs)}</tbody></table></div>')
...
def _class_table(items) -> str:
    ...
    for i, c in enumerate(items):
        ...
        cells = [
            ...
            f'{ck_data_cell(f"""{_html.escape(c.filing_date)}""", align="right", mono=True, tone="dim")}',
            f'{ck_data_cell(f"""{c.class_size:,}""", align="right", mono=True)}',
            f'{ck_data_cell(f"""${c.alleged_damages_m:.1f}M""", align="right", mono=True, tone="neg", weight=700)}',
            f'{ck_data_cell(f"""<span style="display:inline-block;padding:2px 8px;font-size:10px;font-family:JetBrains Mono,monospace;color:{cc};border:1px solid {cc};border-radius:2px;letter-spacing:0.06em">{_html.escape(c.certification_status)}</span>""", align="center")}',
...
def _insurance_table(items) -> str:
    ...
    for i, ins in enumerate(items):
        ...
        cells = [
            f'{ck_data_cell(f"""{_html.escape(ins.layer)}""", mono=True, weight=700)}',
            ...
            f'{ck_data_cell(f"""${ins.limit_m:.1f}M""", align="right", mono=True, tone="pos", weight=700)}',
            f'{ck_data_cell(f"""${ins.attachment_m:.1f}M""", align="right", mono=True, tone="dim")}',
            f'{ck_data_cell(f"""${ins.premium_annual_m:.2f}M""", align="right", mono=True)}',
...
def _history_table(items) -> str:
    ...
    for i, h in enumerate(items):
        rb = panel_alt if i % 2 == 0 else bg
        cells = [
            f'{ck_data_cell(f"""{h.year}""", align="right", mono=True, weight=700)}',
            f'{ck_data_cell(f"""{h.matters_closed}""", align="right", mono=True, tone="acc")}',
            f'{ck_data_cell(f"""${h.alleged_total_m:.1f}M""", align="right", mono=True, weight=700)}',
            ...
            f'{ck_data_cell(f"""{h.paid_to_alleged_ratio * 100:.0f}%""", align="right", mono=True, tone="pos", weight=700)}',
            f'{ck_data_cell(f"""{h.avg_resolution_days}""", align="right", mono=True, tone="dim")}',
        ]
        trs.append(f'<tr>{"".join(cells)}</tr>')
    return (f'<div class="ck-data-table-scroll"><table class="ck-data-table">'
            f'<thead><tr>{ths}</tr></thead><tbody>{"".join(trs)}</tbody></table></div>')
  ...
  <div style="background:{panel_alt};border:1px solid {border};border-left:3px solid {acc};padding:12px 16px;font-size:11px;color:{text_dim};margin-bottom:16px">
    ...
    return chartis_shell(body, "Litigation Tracker", active_nav="/litigation",
        editorial_intro={
            ...
            "headline": "What the litigation tracker page reveals on this deal.",
            "italic_word": "reveals",
        })
