...
def render_rcm_red_flags(params: Dict[str, str]) -> str:
    ...
    if has_inputs:
        ...
        flag_items = "".join(
            ...
            f'<div style="font-size:14px;color:{PALETTE["positive"] if not f["triggered"] else PALETTE["negative"]};margin-top:1px">{"●" if f["triggered"] else "○"}</div>'
            f'<div style="flex:1">'
            f'<div style="font-size:11px;font-weight:600;color:{PALETTE["negative"] if f["triggered"] else P["text_dim"]};font-family:{_SANS}">{html.escape(f["label"])} <span style="font-size:9px;color:{P["text_faint"]}">[{f["weight"]}pts]</span></div>'
