...
def _vendors_table(items) -> str:
    ...
    for i, v in enumerate(items):
        ...
        cells = [
            ...
            f'<td style="text-align:right;padding:5px 10px;font-variant-numeric:tabular-nums;font-family:JetBrains Mono,monospace;font-size:11px;color:{neg}">${v.median_spend_per_deal_k:,.2f}</td>',
...
def _categories_table(items) -> str:
    ...
    for i, c in enumerate(items):
        ...
        cells = [
            ...
            f'<td style="text-align:right;padding:5px 10px;font-variant-numeric:tabular-nums;font-family:JetBrains Mono,monospace;font-size:11px;color:{text_dim}">${c.median_spend_k:,.2f}</td>',
...
def _scorecards_table(items) -> str:
    ...
    for i, s in enumerate(items):
        ...
        cells = [
            ...
            f'<td style="text-align:right;padding:5px 10px;font-variant-numeric:tabular-nums;font-family:JetBrains Mono,monospace;font-size:11px;color:{ot_c};font-weight:700">{s.on_time_delivery_pct * 100:.1f}%</td>',
...
def _pipeline_table(items) -> str:
    ...
    for i, p in enumerate(items):
        ...
        cells = [
            ...
            f'<td style="text-align:right;padding:5px 10px;font-variant-numeric:tabular-nums;font-family:JetBrains Mono,monospace;font-size:11px;color:{l_c};font-weight:700">{p.likelihood_engage_pct * 100:.1f}%</td>',
        ]
        trs.append(f'<tr style="background:{rb}">{"".join(cells)}</tr>')
    return (f'<div style="overflow-x:auto;margin-top:12px"><table style="width:100%;border-collapse:collapse;font-size:11px">'
            f'<thead><tr style="background:{bg}">{ths}</tr></thead><tbody>{"".join(trs)}</tbody></table></div>')
