...
_BOARD_NEEDED = [
    ("director_name", "director (PII)"), ("role", "chair / member / observer"),
    ("committee", "audit / comp / nom"), ("independent", "yes / no"),
    ("meeting_cadence", "monthly / quarterly"),
]
...
def render_board_governance(params: dict = None) -> str:
    ...
    body = page_title + data_required_panel(P, title="Board Governance", needed=_BOARD_NEEDED,
        template="board_governance_template.csv", request_from="Corporate secretary / GC",
        activates="board independence, committee coverage, meeting cadence",
        guide_hint="What board-governance data do I need to upload?") + ck_illustrative_note("figures") + bg_explainer + f"""
