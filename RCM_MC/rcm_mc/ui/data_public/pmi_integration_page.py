...
<div style="padding:20px;max-width:1400px;margin:0 auto">
  <div style="margin-bottom:20px">
    ...
    <p style="font-size:12px;color:{text_dim};margin-top:4px">{r.total_integrations} active integrations · ${r.total_synergy_target_m:.1f}M synergy target · ${r.total_synergy_realized_m:.1f}M realized ({r.weighted_realization_pct * 100:.1f}%) · ${r.total_integration_cost_m:.1f}M integration cost · {r.on_track_count}/{r.total_integrations} on track — {r.corpus_deal_count:,} corpus deals</p>
  </div>
  ...
  <div style="background:{panel_alt};border:1px solid {border};border-left:3px solid {acc};padding:12px 16px;font-size:11px;color:{text_dim};margin-bottom:16px">
    <strong style="color:{text}">PMI Scorecard Summary:</strong> {r.total_integrations} active integrations tracking ${r.total_synergy_target_m:.1f}M synergy target with {r.weighted_realization_pct * 100:.1f}% realization ({r.on_track_count} on track, {r.total_integrations - r.on_track_count} early-stage or behind).
