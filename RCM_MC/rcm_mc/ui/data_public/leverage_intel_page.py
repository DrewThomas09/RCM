...
def _leverage_histogram(points: List[Any], width: int = 440, height: int = 100) -> str:
    ...
    for i in range(n_buckets):
        ...
        if bh_d > 0:
            elements.append(f'<rect x="{bx}" y="{by+bh_p}" width="{max(1,bar_w-1)}" height="{bh_d}" fill="#1F7A75" opacity="0.9"/>')
        if i % 4 == 0:
            elements.append(f'<text x="{bx+bar_w//2}" y="{height-1}" text-anchor="middle" font-family="JetBrains Mono,monospace" font-size="7" fill="#465366">{i*5}%</text>')
    return (
...
def render_leverage_intel() -> str:
    ...
    kpis = (
        ...
        + ck_kpi_block("Lev↔MOIC Corr",
                       f'<span class="mn" style="color:{"#b5321e" if corr<-0.1 else "#0a8a5f" if corr>0.1 else "#b8732a"}">{corr:+.2f}</span>',
...
<div class="ck-panel">
  ...
  <td style="padding:5px 8px;font-family:var(--ck-mono);font-variant-numeric:tabular-nums;text-align:right;
      color:{'#b5321e' if b.loss_rate>0.2 else '#b8732a' if b.loss_rate>0.1 else '#0a8a5f'};">{b.loss_rate*100:.1f}%</td>
...
<div class="ck-panel">
  ...
  <div style="padding:0 16px 10px;font-size:9px;color:#465366;">
