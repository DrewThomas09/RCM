...
def _deals_table(items) -> str:
    ...
    for i, d in enumerate(items):
        ...
        cells = [
            ...
            f'<td style="text-align:right;padding:5px 10px;font-variant-numeric:tabular-nums;font-family:JetBrains Mono,monospace;font-size:11px;color:{text};font-weight:700">${d.equity_check_m:.2f}M</td>',
            f'<td style="text-align:right;padding:5px 10px;font-variant-numeric:tabular-nums;font-family:JetBrains Mono,monospace;font-size:11px;color:{acc};font-weight:700">${d.coinvest_allocation_m:.2f}M</td>',
...
def _lps_table(items) -> str:
    ...
    for i, l in enumerate(items):
        ...
        cells = [
            ...
            f'<td style="text-align:right;padding:5px 10px;font-variant-numeric:tabular-nums;font-family:JetBrains Mono,monospace;font-size:11px;color:{pos};font-weight:700">${l.commitment_m:.2f}M</td>',
            ...
            f'<td style="text-align:right;padding:5px 10px;font-variant-numeric:tabular-nums;font-family:JetBrains Mono,monospace;font-size:11px;color:{text}">${l.avg_check_m:.2f}M</td>',
            f'<td style="text-align:right;padding:5px 10px;font-variant-numeric:tabular-nums;font-family:JetBrains Mono,monospace;font-size:11px;color:{text_dim}">${l.aum_b:.2f}B</td>',
...
def _sectors_table(items) -> str:
    ...
    for i, s in enumerate(items):
        ...
        cells = [
            ...
            f'<td style="text-align:right;padding:5px 10px;font-variant-numeric:tabular-nums;font-family:JetBrains Mono,monospace;font-size:11px;color:{text};font-weight:700">${s.total_equity_m:.2f}M</td>',
            f'<td style="text-align:right;padding:5px 10px;font-variant-numeric:tabular-nums;font-family:JetBrains Mono,monospace;font-size:11px;color:{pos}">${s.allocated_m:.2f}M</td>',
            f'<td style="text-align:right;padding:5px 10px;font-variant-numeric:tabular-nums;font-family:JetBrains Mono,monospace;font-size:11px;color:{P["warning"]}">${s.unallocated_m:.2f}M</td>',
...
def _realizations_table(items) -> str:
    ...
    for i, r in enumerate(items):
        ...
        cells = [
            ...
            f'<td style="text-align:right;padding:5px 10px;font-variant-numeric:tabular-nums;font-family:JetBrains Mono,monospace;font-size:11px;color:{text}">${r.coinvest_invested_m:.2f}M</td>',
            f'<td style="text-align:right;padding:5px 10px;font-variant-numeric:tabular-nums;font-family:JetBrains Mono,monospace;font-size:11px;color:{pos};font-weight:700">${r.coinvest_realized_m:.2f}M</td>',
...
def _fees_table(items) -> str:
    ...
    for i, f in enumerate(items):
        ...
        cells = [
            ...
            f'<td style="text-align:right;padding:5px 10px;font-variant-numeric:tabular-nums;font-family:JetBrains Mono,monospace;font-size:11px;color:{acc};font-weight:700">{f.mgmt_fee_discount * 100:.1f}%</td>',
            f'<td style="text-align:right;padding:5px 10px;font-variant-numeric:tabular-nums;font-family:JetBrains Mono,monospace;font-size:11px;color:{acc}">{f.carry_reduction * 100:.1f}%</td>',
            ...
            f'<td style="text-align:right;padding:5px 10px;font-variant-numeric:tabular-nums;font-family:JetBrains Mono,monospace;font-size:11px;color:{pos};font-weight:700">${f.realized_savings_m:.2f}M</td>',
        ]
        trs.append(f'<tr style="background:{rb}">{"".join(cells)}</tr>')
    return (f'<div style="overflow-x:auto;margin-top:12px"><table style="width:100%;border-collapse:collapse;font-size:11px">'
            f'<thead><tr style="background:{bg}">{ths}</tr></thead><tbody>{"".join(trs)}</tbody></table></div>')
...
def _capacity_table(items) -> str:
    ...
    for i, c in enumerate(items):
        ...
        cells = [
            ...
            f'<td style="text-align:right;padding:5px 10px;font-variant-numeric:tabular-nums;font-family:JetBrains Mono,monospace;font-size:11px;color:{text}">${c.total_equity_m:.2f}M</td>',
            f'<td style="text-align:right;padding:5px 10px;font-variant-numeric:tabular-nums;font-family:JetBrains Mono,monospace;font-size:11px;color:{text_dim}">${c.sponsor_check_m:.2f}M</td>',
            f'<td style="text-align:right;padding:5px 10px;font-variant-numeric:tabular-nums;font-family:JetBrains Mono,monospace;font-size:11px;color:{text_dim}">${c.anchor_coinvest_m:.2f}M</td>',
            f'<td style="text-align:right;padding:5px 10px;font-variant-numeric:tabular-nums;font-family:JetBrains Mono,monospace;font-size:11px;color:{acc};font-weight:700">${c.remaining_capacity_m:.2f}M</td>',
            f'<td style="text-align:right;padding:5px 10px;font-variant-numeric:tabular-nums;font-family:JetBrains Mono,monospace;font-size:11px;color:{pos};font-weight:700">${c.indicative_demand_m:.2f}M</td>',
...
def render_coinvest_pipeline(params: dict = None) -> str:
    ...
    kpi_strip = (
        ...
        ck_kpi_block("Equity Pipeline", f"${r.total_equity_pipeline_m:,.2f}M", "", "") +
        ck_kpi_block("Coinvest Available", f"${r.total_coinvest_available_m:,.2f}M", "", "") +
        ck_kpi_block("Allocated", f"${r.total_coinvest_allocated_m:,.2f}M", "", "") +
...
<div style="padding:20px;max-width:1400px;margin:0 auto">
  <div style="margin-bottom:20px">
    ...
    <p style="font-size:12px;color:{text_dim};margin-top:4px">{r.active_opportunities} active opportunities · ${r.total_equity_pipeline_m:,.2f}M equity pipeline · ${r.total_coinvest_available_m:,.2f}M coinvest capacity · {r.active_lp_count} active LPs · historical {r.historical_avg_moic:.2f}x MOIC / {r.historical_avg_irr_pct:.1f}% IRR — {r.corpus_deal_count:,} corpus deals</p>
  </div>
