...
def _moic_spread_bar(p25: float, p50: float, p75: float, width: int = 120) -> str:
    ...
    return (
        ...
        f'<rect x="0" y="4" width="{width}" height="4" rx="0" fill="#d6cfc3"/>'
        f'<rect x="{x25}" y="2" width="{max(1,x75-x25)}" height="8" rx="0" fill="#1e3a5c"/>'
        f'<line x1="{x50}" y1="0" x2="{x50}" y2="12" stroke="#2fb3ad" stroke-width="2"/>'
        f'</svg>'
    )
...
def _sparkline(vintage_moic: Dict[int, List[float]], width: int = 80, height: int = 20) -> str:
    ...
    if not vintage_moic:
        return f'<svg width="{width}" height="{height}"><line x1="0" y1="{height//2}" x2="{width}" y2="{height//2}" stroke="#d6cfc3" stroke-width="1"/></svg>'
    ...
    return (
        ...
        f'<polyline points="{coords}" fill="none" stroke="#2fb3ad" stroke-width="1.2"/>'
        f'</svg>'
    )
...
def _scatter_svg(stats: List[Any], width: int = 480, height: int = 320) -> str:
    ...
    for s in stats:
        ...
        color = "#22c55e" if s.moic_p50 >= 3.0 else ("#2fb3ad" if s.moic_p50 >= 2.0 else "#b8732a" if s.moic_p50 >= 1.5 else "#b5321e")
...
<div class="ck-panel">
  ...
  <div style="padding:12px 16px;overflow-x:auto;">
    ...
    sort_links = "".join(
        ...
        f'color:{"#2fb3ad" if k==sort_by else "#7a8699"};text-decoration:none;">{k.replace("_"," ")}</a>'
...
<div class="ck-panel">
  <div style="padding:8px 16px;display:flex;align-items:center;gap:16px;flex-wrap:wrap;">
    ...
    <div>
      ...
      {"".join(
        ...
        f'color:{"#2fb3ad" if n==min_deals else "#7a8699"};text-decoration:none;">{n}+</a>'
        for n in (1, 3, 5, 10)
      )}
    </div>
  </div>
  ...
  <td style="padding:5px 8px;font-family:var(--ck-mono);font-variant-numeric:tabular-nums;text-align:right;color:#2fb3ad;">{s.sharpe_proxy:.2f}</td>
