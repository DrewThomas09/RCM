...
def _contracts_table(items) -> str:
    ...
    for i, c in enumerate(items):
        ...
        cells = [
            ...
            f'<td style="text-align:right;padding:5px 10px;font-variant-numeric:tabular-nums;font-family:JetBrains Mono,monospace;font-size:11px;color:{pos};font-weight:700">${c.annual_revenue_m:.2f}M</td>',
...
def _negotiations_table(items) -> str:
    ...
    for i, n in enumerate(items):
        ...
        cells = [
            ...
            f'<td style="text-align:right;padding:5px 10px;font-variant-numeric:tabular-nums;font-family:JetBrains Mono,monospace;font-size:11px;color:{pos};font-weight:700">${n.revenue_at_stake_m:.2f}M</td>',
        ]
        trs.append(f'<tr style="background:{rb}">{"".join(cells)}</tr>')
    return (f'<div style="overflow-x:auto;margin-top:12px"><table style="width:100%;border-collapse:collapse;font-size:11px">'
            f'<thead><tr style="background:{bg}">{ths}</tr></thead><tbody>{"".join(trs)}</tbody></table></div>')
...
def _network_table(items) -> str:
    ...
    for i, n in enumerate(items):
        ...
        cells = [
            ...
            f'<td style="text-align:right;padding:5px 10px;font-variant-numeric:tabular-nums;font-family:JetBrains Mono,monospace;font-size:11px;color:{c_c};font-weight:700">{n.time_distance_compliance_pct * 100:.1f}%</td>',
...
def _optimization_table(items) -> str:
    ...
    for i, o in enumerate(items):
        ...
        cells = [
            ...
            f'<td style="text-align:right;padding:5px 10px;font-variant-numeric:tabular-nums;font-family:JetBrains Mono,monospace;font-size:11px;color:{pos};font-weight:700">${o.annualized_value_m:.2f}M</td>',
...
def render_payer_contracts(params: dict = None) -> str:
    ...
    kpi_strip = (
        ...
        ck_kpi_block("Annual Revenue", f"${r.total_annual_revenue_m:,.2f}M", "", "") +
        ...
        ck_kpi_block("Revenue @ Renegotiation", f"${r.revenue_at_renegotiation_m:.2f}M", "", "") +
...
<div style="padding:20px;max-width:1400px;margin:0 auto">
  <div style="margin-bottom:20px">
    ...
    <p style="font-size:12px;color:{text_dim};margin-top:4px">{r.total_contracts} contracts · ${r.total_annual_revenue_m:,.2f}M annual revenue · weighted {r.weighted_avg_escalator_pct * 100:.2f}% escalator · {r.contracts_expiring_12mo} expiring ≤12 months · ${r.revenue_at_renegotiation_m:.2f}M at renegotiation — {r.corpus_deal_count:,} corpus deals</p>
  </div>
