...
def _moic_html(v: Optional[float]) -> str:
    if v is None:
        return "—"
    color = "var(--theme-negative,#ef4444)" if v < 1.0 else ("var(--theme-positive,#22c55e)" if v >= 2.5 else "var(--theme-warning,#f59e0b)")
    return (
        f'<span style="font-family:var(--ck-mono);font-variant-numeric:tabular-nums;'
...
def _irr_html(v: Optional[float]) -> str:
    if v is None:
        return "—"
    color = "var(--theme-negative,#ef4444)" if v < 0.10 else ("var(--theme-positive,#22c55e)" if v >= 0.20 else "var(--theme-warning,#f59e0b)")
    return (
        f'<span style="font-family:var(--ck-mono);font-variant-numeric:tabular-nums;'
...
def _pct_rank_bar(rank: float, width: int = 120) -> str:
    ...
    color = "var(--theme-positive,#22c55e)" if rank >= 60 else ("var(--theme-warning,#f59e0b)" if rank >= 35 else "var(--theme-negative,#ef4444)")
    return (
        ...
        for row in sens:
            ...
            for mult_key in sorted([k for k in row if k != "hold_years"]):
                ...
                if isinstance(r, dict):
                    ...
                    color = "var(--theme-negative,#ef4444)" if (moic or 0) < 1.0 else ("var(--theme-positive,#22c55e)" if (moic or 0) >= 2.5 else "#e2e8f0")
                    cells.append(
...
def _corpus_benchmark_panel(result: Any, corpus: List[Dict[str, Any]]) -> str:
    ...
    delta_color = "var(--theme-positive,#22c55e)" if moic_vs_p50 > 0 else "var(--theme-negative,#ef4444)"
...
<div class="ck-panel">
  ...
  <div style="padding:12px 16px;display:grid;grid-template-columns:1fr 1fr 1fr 1fr 1fr;gap:1px;background:var(--ck-border);">
    ...
    <div style="background:var(--ck-panel-alt);padding:10px 12px;">
      ...
      <div style="font-family:var(--ck-mono);font-size:14px;color:var(--theme-negative,#ef4444);font-variant-numeric:tabular-nums;">{loss*100:.1f}%</div>
    </div>
    <div style="background:var(--ck-panel-alt);padding:10px 12px;">
...
def render_underwriting(
    ...
    try:
        ...
        results_html = f'<div class="ck-panel"><div style="padding:12px;color:{PALETTE["negative"]};">Error: {_html.escape(str(exc))}</div></div>'
    ...
    kpis = (
        ...
        + ck_kpi_block("Model MOIC",
                       f'<span class="mn" style="color:{PALETTE["positive"] if (result.gross_moic or 0) >= 2.5 else PALETTE["warning"]}">'
