...
def _funnel_table(items) -> str:
    ...
    ths = "".join(f'<th style="text-align:{a};padding:6px 10px;border-bottom:1px solid {border};font-size:11px;color:{text_dim};letter-spacing:0.05em">{c}</th>' for c, a in cols)
    trs = []
...
def _channels_table(items) -> str:
    ...
    for i, c in enumerate(items):
        ...
        cells = [
            f'<td style="text-align:left;padding:5px 10px;font-family:JetBrains Mono,monospace;font-size:13px;color:{text};font-weight:700">{_html.escape(c.channel)}</td>',
            f'<td style="text-align:right;padding:5px 10px;font-variant-numeric:tabular-nums;font-family:JetBrains Mono,monospace;font-size:13px;color:{text};font-weight:600">{c.leads_ltm}</td>',
...
def _intermediaries_table(items) -> str:
    ...
    for i, m in enumerate(items):
        ...
        cells = [
            f'<td style="text-align:left;padding:5px 10px;font-family:JetBrains Mono,monospace;font-size:13px;color:{text};font-weight:700">{_html.escape(m.firm)}</td>',
            ...
            f'<td style="text-align:right;padding:5px 10px;font-variant-numeric:tabular-nums;font-family:JetBrains Mono,monospace;font-size:13px;color:{text}">{m.contacts_primary}</td>',
            f'<td style="text-align:right;padding:5px 10px;font-variant-numeric:tabular-nums;font-family:JetBrains Mono,monospace;font-size:13px;color:{acc}">{m.deals_shown_ltm}</td>',
            f'<td style="text-align:right;padding:5px 10px;font-variant-numeric:tabular-nums;font-family:JetBrains Mono,monospace;font-size:13px;color:{pos};font-weight:700">{m.deals_closed}</td>',
            f'<td style="text-align:right;padding:5px 10px;font-variant-numeric:tabular-nums;font-family:JetBrains Mono,monospace;font-size:13px;color:{c_c};font-weight:700">{m.conversion_rate_pct * 100:.2f}%</td>',
            f'<td style="text-align:right;padding:5px 10px;font-variant-numeric:tabular-nums;font-family:JetBrains Mono,monospace;font-size:13px;color:{text_dim}">{m.reverse_inquiry_count}</td>',
            f'<td style="text-align:right;padding:5px 10px;font-variant-numeric:tabular-nums;font-family:JetBrains Mono,monospace;font-size:13px;color:{s_c};font-weight:700">{m.relationship_score:.1f}</td>',
        ]
        trs.append(f'<tr style="background:{rb}">{"".join(cells)}</tr>')
    return (f'<div style="overflow-x:auto;margin-top:12px"><table style="width:100%;border-collapse:collapse;font-size:13px">'
            f'<thead><tr style="background:{bg}">{ths}</tr></thead><tbody>{"".join(trs)}</tbody></table></div>')
...
def _proprietary_table(items) -> str:
    ...
    for i, p in enumerate(items):
        ...
        cells = [
            f'<td style="text-align:left;padding:5px 10px;font-family:JetBrains Mono,monospace;font-size:13px;color:{text};font-weight:700">{_html.escape(p.target)}</td>',
...
def _team_table(items) -> str:
    ...
    for i, t in enumerate(items):
        ...
        cells = [
            f'<td style="text-align:left;padding:5px 10px;font-family:JetBrains Mono,monospace;font-size:13px;color:{text};font-weight:700">{_html.escape(t.partner)}</td>',
...
def _closed_bridge_table(items) -> str:
    ...
    for i, c in enumerate(items):
        rb = panel_alt if i % 2 == 0 else bg
        cells = [
            f'<td style="text-align:left;padding:5px 10px;font-family:JetBrains Mono,monospace;font-size:13px;color:{text};font-weight:700">{_html.escape(c.deal)}</td>',
