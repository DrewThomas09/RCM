...
def _waterfall_svg(components, fund_moic: float) -> str:
    ...
    colors = [P["accent"], "#14b8a6", "#a78bfa", "var(--theme-warning,#f59e0b)", "#f97316"]
...
def _deal_attr_table(attrs) -> str:
    ...
    for i, a in enumerate(attrs[:30]):   # top 30 deals
        ...
        cells = [
            ...
            f'<td style="text-align:right;padding:5px 10px;font-variant-numeric:tabular-nums;font-family:JetBrains Mono,monospace;font-size:11px;color:{text_dim}">${a.entry_ev_mm:,.2f}</td>',
            f'<td style="text-align:right;padding:5px 10px;font-variant-numeric:tabular-nums;font-family:JetBrains Mono,monospace;font-size:11px;color:{text}">${a.exit_ev_mm:,.2f}</td>',
