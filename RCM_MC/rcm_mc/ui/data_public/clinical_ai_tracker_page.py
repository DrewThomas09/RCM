...
def render_clinical_ai_tracker(params: dict = None) -> str:
    ...
    body = page_title + data_required_panel(P, title="Clinical AI", needed=[("use_case","clinical AI tool / use-case"),("vendor","vendor"),("adoption_status","pilot / scaled / none"),("validation","validation status"),("risk_level","high / med / low")], template="ai_operating_model_template.csv", request_from="CMIO / clinical informatics", activates="clinical-AI tool inventory, adoption, and validation tracking", guide_hint="What clinical-AI tooling data do I need to upload?") + ck_illustrative_note("figures") + cai_explainer + f"""
