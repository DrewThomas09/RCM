...
def _init_table(items) -> str:
    ...
    for i, it in enumerate(items):
        ...
        cells = [
            ...
            f'<td style="text-align:right;padding:5px 10px;font-variant-numeric:tabular-nums;font-family:JetBrains Mono,monospace;font-size:11px;color:{adopt_c};font-weight:700">{it.adoption_pct * 100:.1f}%</td>',
...
def render_ai_operating_model(params: dict = None) -> str:
    ...
    kpi_strip = (
        ...
        ck_kpi_block("Blended ROI", f"{r.blended_roi_pct:.2f}x", "", "") +
...
<div style="padding:20px;max-width:1400px;margin:0 auto">
  ...
  <div style="background:{panel_alt};border:1px solid {border};border-left:3px solid {gov_c};padding:14px 18px;margin-bottom:16px;font-size:13px;font-family:JetBrains Mono,monospace">
    ...
    <div style="color:{gov_c};font-weight:700;font-size:14px">Blended ROI {r.blended_roi_pct:.2f}x · {r.initiatives_in_prod} initiatives in production · Governance posture {_html.escape(r.governance_risk_tier.upper())}</div>
  ...
  <div style="background:{panel_alt};border:1px solid {border};border-left:3px solid {acc};padding:12px 16px;font-size:11px;color:{text_dim};margin-bottom:16px">
    <strong style="color:{text}">AI Operating Thesis:</strong> ${r.total_annual_ai_spend_mm:,.2f}M annual spend returns ${r.total_annual_savings_mm + r.total_revenue_lift_mm:,.2f}M value at {r.blended_roi_pct:.2f}x ROI.
