...
def _hold_histogram(
    ...
    n_lbl = f'<text x="{pad_l+3}" y="{pad_t+9}" font-size="7.5" fill="#465366">n={len(hold_vals)}</text>'
...
def _hold_moic_scatter(
    ...
    dots = "".join(
        f'<circle cx="{tx(x):.1f}" cy="{ty(y):.1f}" r="2.2" fill="#1F7A75" fill-opacity="0.6"/>'
        for x, y in pts
...
def _kpi_bar(stats: Dict[str, Any]) -> str:
    ...
    def moic_html(v):
        ...
        color = "#0a8a5f" if v >= 2.5 else ("#b8732a" if v >= 1.5 else "#b5321e")
...
def _sector_table_panel(rows: List[Dict[str, Any]]) -> str:
    ...
    for i, r in enumerate(rows):
        ...
        moic_color = "#0a8a5f" if (r["moic_p50"] or 0) >= 2.5 else ("#b8732a" if (r["moic_p50"] or 0) >= 1.5 else "#b5321e")
        moic_html = (
...
def _vintage_panel(vintage_data: Dict[int, Dict[str, Any]]) -> str:
    ...
    for i, yr in enumerate(years):
        ...
        bh = pct * ph
        by = pad_t + ph - bh
        color = "#0a8a5f" if pct >= 0.80 else ("#b8732a" if pct >= 0.50 else "#1F7A75")
        bars.append(
