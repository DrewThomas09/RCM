...
# v1 compat font-stack constants
_MONO = "'JetBrains Mono', 'SF Mono', 'Fira Code', Consolas, monospace"
_SANS = "'Inter Tight', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif"
...
P = {
    ...
    # v1 compat aliases
    "accent":     "#2fb3ad",   # was P['teal'] in v2
    "border":     "#d6cfc3",   # was P['rule'] in v2
    "border_dim": "#c5bdae",   # was P['rule_2'] in v2
}
...
# ---------------------------------------------------------------------------
# Legacy format aliases — preserved for callers written against v1 dark shell
# ---------------------------------------------------------------------------

def ck_fmt_num(value: object, decimals: int = 1, suffix: str = "", na: str = "—") -> str:
    """v1 compat: format a number with tabular-nums span."""
    if value is None:
    ...
    try:
        ...
        if suffix == "x" and f < 1.0:
            col = " neg"
        elif suffix == "x" and f >= 2.5:
            col = " pos"
        return f'<span class="mn{col}">{f:,.{decimals}f}{_html.escape(suffix)}</span>'
    except (TypeError, ValueError):
        return f'<span class="faint">{na}</span>'


def ck_fmt_pct(value: object, decimals: int = 1, signed: bool = False) -> str:
    """v1 compat: format a 0–1 fraction as a percentage."""
    if value is None:
        return '<span class="faint">—</span>'
    try:
        f = float(value) * 100.0  # type: ignore[arg-type]
...
def ck_fmt_moic(value: object) -> str:
    """v1 compat: format MOIC with color coding."""
    if value is None:
        return '<span class="faint">—</span>'
    try:
        f = float(value)  # type: ignore[arg-type]
...
def ck_grade_badge(grade: Optional[str]) -> str:
    """v1 compat: HTML grade badge (A/B/C/D)."""
    if not grade:
...
def ck_section_header(
    title: str,
    subtitle: str = "",    # legacy 2nd positional arg (v1 compat)
    count: object = None,  # legacy 3rd positional arg (v1 compat)
    *,
    eyebrow: Optional[str] = None,
    code: Optional[str] = None,
) -> str:
    effective_eyebrow = eyebrow or subtitle or ""
    eb = f'<div class="sc-eyebrow">{_esc(effective_eyebrow)}</div>' if effective_eyebrow else ""
    count_html = f'<span class="ck-section-code">{_esc(str(count))}</span>' if count is not None else ""
...
def ck_table(
    ...
    *,
    dense: bool = False,
    # Legacy compat kwargs — accepted but not rendered
    caption: str = "",
    sortable: bool = False,
    id: str = "",
) -> str:
...
def ck_kpi_block(
    label: str,
    value: str,
    unit: str = "",        # legacy 3rd positional arg (v1 compat)
    delta: str = "",       # legacy 4th positional arg (v1 compat)
    *,
    trend: Optional[str] = None,
    sub: Optional[str] = None,
    code: Optional[str] = None,
) -> str:
    effective_trend = trend or delta or ""
    effective_sub = sub or unit or ""
    trend_html = ""
    if effective_trend:
        tone = "positive" if effective_trend.startswith("+") else "negative" if effective_trend.startswith("-") else "neutral"
        trend_html = f'<span class="ck-kpi-trend tone-{tone}">{_esc(effective_trend)}</span>'
    sub_html = f'<div class="ck-kpi-sub">{_esc(effective_sub)}</div>' if effective_sub else ""
...
def ck_related_views(items: List[tuple], label: str = "Related views") -> str:
    if not items:
        return ""
    links = " · ".join(
        f'<a href="{href}" class="ck-related-link">{_esc(lbl)}</a>'
        for (lbl, href) in items
    )
    return (
        f'<div class="ck-related">'
        f'<span class="ck-related-label">{_esc(label)}</span>'
        f'{links}'
        f'</div>'
    )
...
def _topbar(active_nav: Optional[str], user_initials: str = "AT") -> str:
    ...
    active_attr = f' data-active="{_esc(active_nav)}"' if active_nav else ""
    return (
        f'<header class="ck-topbar"{active_attr}>'
        '<div class="ck-topbar-inner">'
        '<a href="/" class="ck-wordmark"><span class="ck-wordmark-mark"></span>SeekingChartis</a>'
        f'<nav class="ck-nav" aria-label="Main navigation">{links}</nav>'
...
def chartis_shell(
    ...
    *,
    ...
    # Legacy compat — silently accepted but not rendered in v2
    subtitle: str = "",
    extra_css: str = "",
    extra_js: str = "",
) -> str:
    ...
    ``subtitle``, ``extra_css``, and ``extra_js`` are accepted for
    backward-compat with callers written against the v1 dark shell.
    """
    if not UI_V2_ENABLED:
        try:
            ...
            return _legacy.chartis_shell(
                ...
                subtitle=subtitle, extra_css=extra_css, extra_js=extra_js,
            )
        except Exception:
    ...
    extra_css_tag = f"<style>{extra_css}</style>" if extra_css else ""
    extra_js_tag = f"<script>{extra_js}</script>" if extra_js else ""
    palette_html = ""
    ...
    subtitle_bar = (
        f'<div class="ck-subtitle">{_esc(subtitle)}</div>' if subtitle else ""
    )
    return (
        "<!doctype html>"
        '<html lang="en"><head>'
        ...
        f'<main class="ck-main">{debug_tag}{subtitle_bar}{body_html}</main>'
        f"{palette_html}"
        f"{_PALETTE_JS}"
        f"{extra_js_tag}"
        "</body></html>"
