...
class RCMHandler(BaseHTTPRequestHandler):
    ...
    def _route_deal_surface(self, ccn: str, slug: str) -> None:
        ...
        if slug == "lbo":
            return self._send_html(render_deal_lbo(ccn, hospital))
        if slug == "dcf":
            return self._send_html(render_deal_dcf(ccn, hospital))
        if slug == "comp-intel":
            return self._send_html(render_deal_comp_intel(ccn, hospital))
        if slug == "ml":
            return self._send_html(render_deal_ml(ccn, hospital))
        if slug == "denial":
            return self._send_html(render_deal_denial(ccn, hospital))
        if slug == "returns":
            return self._send_html(render_deal_returns(ccn, hospital))
        if slug == "trends":
        ...
        if slug == "waterfall":
            return self._send_html(render_deal_waterfall(ccn, hospital))
        if slug == "playbook":
            return self._send_html(render_deal_playbook(ccn, hospital))
        if slug == "ic-memo":
            return self._send_html(render_deal_ic_memo(ccn, hospital))
        if slug == "memo-auto":
            return self._send_html(render_deal_memo_auto(ccn, hospital))
        if slug == "market":
    ...
    def _route_login_page_legacy(self) -> None:
        ...
        * {{ box-sizing: border-box; margin: 0; padding: 0; }}
        ...
        page = (
            ...
            # Offline-first: no external font CDN (CSP blocks it + it
            # leaks egress). Local fallback stacks render the login chrome.
            f'<style>{css}</style>'
            '</head><body>'
            f'{top_bar}'
