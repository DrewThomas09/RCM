...
_SOURCES: List[DataSourceContext] = [
    ...
    _s("sec_edgar", "SEC EDGAR", "Public SEC filings (for public comparables / "
       ...
       related_metrics=["revenue", "ebitda", "ev_to_ebitda",
                       "operating_margin", "leverage"],
    ...
    _s("irs_form_990", "IRS Form 990", "Nonprofit tax filings (for nonprofit "
       ...
       related_metrics=["revenue", "operating_margin"],
    ...
    _s("cms_ffs_provider_enrollment", "CMS FFS Provider Enrollment",
       ...
       used_for=["Provider-supply density / market-supply backdrop.",
                 "providers-per-1k metric in the Geographic Intelligence suite."],
       related_routes=["/provider-network", "/workforce-planning", "/market-intel/geo",
                       "/state-compare", "/state-rankings", "/geo-intel"],
       related_metrics=["bed_count", "provider_productivity"],
    ...
    _s("cms_chow", "CMS Change of Ownership (CHOW)",
       ...
       related_metrics=["hhi", "concentration_ratio"],
    ...
    _s("cms_ma_geo", "CMS Medicare Advantage Geographic Enrollment",
       ...
       related_routes=["/payer-concentration", "/risk-adjustment", "/medicaid-unwinding",
                       "/market-intel/geo"],
       related_metrics=["medicare_exposure", "payer_mix"],
    ...
    _s("cms_hcahps", "CMS HCAHPS Patient Experience",
       ...
       related_metrics=["cms_star_rating", "benchmark_percentile"],
    ...
    _s("hrsa_hpsa", "HRSA Health Professional Shortage Areas",
       ...
       related_metrics=["physician_attrition", "labor_cost_ratio"],
    ...
    _s("cms_mssp_aco", "CMS MSSP ACO Landscape",
       ...
       related_metrics=["benchmark_percentile", "model_estimate"],
    ...
    _s("civhc_rbp", "CIVHC Commercial-vs-Medicare (CO APCD)",
       ...
       related_metrics=["commercial_payer_exposure", "payer_mix",
                       "underpayment_rate"],
    ...
    _s("chr_county_demographics", "County Demographics (Census/ACS via CHR)",
       ...
       source_confidence=_DOCD, data_confidence=_PUB, ic_ready=False),
    _s("cbsa_crosswalk", "CBSA Metro Demographics (OMB delineation × county ACS)",
       "Metro/micro-area (CBSA) demographics rolled up from county ACS via the "
       "OMB July-2023 CBSA delineation — population (sum), age 65+/uninsured/"
       "rural (population-weighted), median income (weighted estimate).",
       _T.PUBLIC_DATASET,
       aliases=["cbsa", "metro markets", "metro demographics", "omb delineation",
                "core-based statistical area"],
       update_cadence="Per OMB delineation vintage.", freshness_lag="ACS 5-yr; OMB 2023.",
       used_for=["Metro-level market sizing (/metro-markets); the real counterpart to the illustrative /geo-market."],
       related_routes=["/metro-markets", "/geo-intel"],
       strengths=["Real county→CBSA rollup; 918 CBSAs; public-domain crosswalk."],
       limitations=["Median income is a population-weighted estimate, not a true CBSA median; area-level."],
       provenance_notes="rcm_mc/data/cbsa_demographics.py; scripts/ingest_cbsa_crosswalk.py; vendor/cbsa_crosswalk/; registry cbsa_crosswalk.",
       source_confidence=_DOCD, data_confidence=_PUB, ic_ready=False),
    _s("cms_partd_drug_spending", "CMS Part D Spending by Drug",
       ...
       related_metrics=["medicare_exposure"],
...
_PARTNER_SOURCE_ALIAS_EXTENSIONS: Dict[str, List[str]] = {
    ...
    "civhc_rbp": ["apcd", "reference pricing", "all payer claims",
                  "co apcd", "colorado apcd", "cost of care"],
