"""The canonical QPP tables + an idempotent SQLite store.

Three normalization targets for this workstream:

  qpp_clinician     — one row per clinician x performance year, keyed by
                      ``npi_year`` = ``{npi}:{year}``.
  qpp_organization  — one row per clinician x year x practice
                      organization, keyed by ``org_key`` =
                      ``{npi}:{year}:{idx}``.
  qpp_benchmark     — one row per quality-measure benchmark x submission
                      method x performance year, keyed by
                      ``benchmark_key``.

Every row keeps the source payload verbatim in a ``raw`` JSON column —
the QPP API versions its response shape via Accept headers, so the
normalized columns are the stable read path and ``raw`` is the honest
fallback when a field this connector doesn't map is needed.
...
TABLES: Dict[str, TableDef] = {
    "qpp_clinician": TableDef(
        "qpp_clinician", "npi_year",
        ("npi_year", "npi", "year", "first_name", "middle_name", "last_name",
         "npi_type", "newly_enrolled", "specialty_description",
         "specialty_type", "specialty_category", "is_maqi",
         "n_organizations", "raw", *_META),
    ),
    "qpp_organization": TableDef(
        "qpp_organization", "org_key",
        ("org_key", "npi", "year", "org_idx", "org_name", "facility_based",
         "apms_count", "virtual_groups_count", "raw", *_META),
    ),
    "qpp_benchmark": TableDef(
        "qpp_benchmark", "benchmark_key",
        ("benchmark_key", "measure_id", "performance_year", "benchmark_year",
         "submission_method", "status", "is_topped_out", "is_inverse",
         "deciles", "raw", *_META),
    ),
}

CANONICAL_TABLES: Tuple[str, ...] = (
    "qpp_clinician", "qpp_organization", "qpp_benchmark")


class QppStore:
    """Thin SQLite wrapper: schema bootstrap + idempotent batch upsert.

    The only module that talks to the QPP SQLite file directly, mirroring
    ...
    def ensure_schema(self) -> None:
        ...
        cur.execute("CREATE INDEX IF NOT EXISTS ix_qpp_clin_npi "
                    "ON qpp_clinician(npi)")
        cur.execute("CREATE INDEX IF NOT EXISTS ix_qpp_org_npi "
                    "ON qpp_organization(npi)")
        cur.execute("CREATE INDEX IF NOT EXISTS ix_qpp_bench_measure "
                    "ON qpp_benchmark(measure_id, performance_year)")
        self.conn.commit()
