"""A standalone stdlib HTTP surface for the QPP slice.
...
registered QPP datasets and the lookup handlers. The same registry +
...
supporting the `field__op` grammar (`measure_id__like=00%`). The caller
never sees the QPP API's native shapes — they were absorbed at ingest.
"""
...
from .tables import TABLES, QppStore
...
_CONNECTOR_NAME = "qpp"
_CONNECTOR_LABEL = "CMS Quality Payment Program (MIPS/APM)"


def connector_status(store: QppStore) -> Dict[str, Any]:
...
def build_handler(store: QppStore):
    ...
    class QppHandler(BaseHTTPRequestHandler):
        server_version = "qpp-connector/1.0"
        ...
        def _route(self, parts: List[str], qs: Dict[str, List[str]]) -> None:
            ...
            # /v1/lookup/qpp-clinician/{npi} , /v1/lookup/qpp-organizations/{npi},
            # /v1/lookup/qpp-benchmarks/{measure_id}?year=
            if len(parts) == 4 and parts[0] == "v1" and parts[1] == "lookup":
                key = parts[3]
                if parts[2] == "qpp-clinician":
                    return self._send(
                        200, lookups["/v1/lookup/qpp-clinician/{npi}"](key))
                if parts[2] == "qpp-organizations":
                    return self._send(
                        200,
                        lookups["/v1/lookup/qpp-organizations/{npi}"](key))
                if parts[2] == "qpp-benchmarks":
                    year = qs.get("year", [""])[0]
                    return self._send(
                        200,
                        lookups["/v1/lookup/qpp-benchmarks/{measure_id}"](
                            key, year))
...
def make_server(store: QppStore, *, host: str = "127.0.0.1", port: int = 0
...
def serve(db_path: str, *, host: str = "127.0.0.1", port: int = 8100) -> None:  # pragma: no cover
    """Open the store and serve forever (CLI entry)."""
    store = QppStore(db_path)
    server, bound = make_server(store, host=host, port=port)
    print(f"QPP /v1 surface on http://{host}:{bound}  (Ctrl-C to stop)")
    try:
