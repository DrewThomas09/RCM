"""A standalone stdlib HTTP surface for the CMS Open Data slice.
...
every registered CMS Open Data dataset and the five lookup handlers.
...
supporting the `field__op` grammar (`tot_clms__gte=100`). The caller
never sees the data API's native size/offset — it was absorbed at ingest.
"""
...
_CONNECTOR_NAME = "cms_open_data"
_CONNECTOR_LABEL = "CMS Open Data (data.cms.gov data-api)"


def connector_status(store: CmsOpenDataStore) -> Dict[str, Any]:
...
def build_handler(store: CmsOpenDataStore):
    ...
    # Dispatch lookups by their noun segment so adding a route in
    # lookup.py never needs an edit here.
    lookups_by_noun = {
        tmpl.strip("/").split("/")[2]: fn for tmpl, fn in lookups.items()
    }

    class CmsOpenDataHandler(BaseHTTPRequestHandler):
        server_version = "cms-open-data-connector/1.0"
        ...
        def _route(self, parts: List[str], qs: Dict[str, List[str]]) -> None:
            ...
            if len(parts) == 4 and parts[0] == "v1" and parts[1] == "lookup":
                handler = lookups_by_noun.get(parts[2])
                if handler is not None:
                    return self._send(200, handler(parts[3]))
            self._send(404, {"error": f"no route for /{'/'.join(parts)}"})

    return CmsOpenDataHandler


def make_server(store: CmsOpenDataStore, *, host: str = "127.0.0.1", port: int = 0
...
def serve(db_path: str, *, host: str = "127.0.0.1", port: int = 8103) -> None:  # pragma: no cover
    """Open the store and serve forever (CLI entry)."""
    store = CmsOpenDataStore(db_path)
    server, bound = make_server(store, host=host, port=port)
    print(f"CMS Open Data /v1 surface on http://{host}:{bound}  (Ctrl-C to stop)")
    try:
