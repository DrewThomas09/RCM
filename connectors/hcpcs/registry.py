"""Declarative dataset registry — one row per HCPCS dataset.
...
(:mod:`connectors.hcpcs.query`) auto-exposes anything in this registry.

Each registry row carries a ``source_filter`` (the ``source_endpoint``
value) so a dataset query returns exactly that code set's slice — the
estate's documented shared-table pattern, kept even with a single Level
II endpoint so a future companion code set lands as a row, not a schema
change.

All rows are tagged ``source='hcpcs'`` so the central registry can ingest
them without the HCPCS workstream touching the router core.
"""
...
def _row(spec: EndpointSpec) -> RegistryRow:
    ...
    return RegistryRow(
        ...
        date_field="",                 # HCPCS is a reference dimension, no date
    )


def registry_rows() -> List[RegistryRow]:
    """Every HCPCS dataset as a declarative registry row."""
