"""A standalone stdlib HTTP surface for the HCPCS slice.
...
registered HCPCS dataset and the lookup/search handlers. The same
registry + `v1_handlers` map drive the unified estate server; this
standalone surface proves the contract and is testable end to end.
...
supporting the `field__op` grammar (`code__like=J9%`). The caller never
sees the NLM API's native paging — it was absorbed at ingest.
"""
...
from .tables import TABLES, HcpcsStore

_RESERVED = {"select", "sort", "limit", "offset", "group_by", "metric", "q"}
...
_CONNECTOR_NAME = "hcpcs"
_CONNECTOR_LABEL = "HCPCS Level II (NLM Clinical Tables)"


def connector_status(store: HcpcsStore) -> Dict[str, Any]:
...
def build_handler(store: HcpcsStore):
    ...
    class HcpcsHandler(BaseHTTPRequestHandler):
        server_version = "hcpcs-connector/1.0"
        ...
        def _route(self, parts: List[str], qs: Dict[str, List[str]]) -> None:
            ...
            # /v1/lookup/hcpcs/{code} , /v1/lookup/hcpcs-section/{s},
            ...
            if len(parts) == 4 and parts[0] == "v1" and parts[1] == "lookup":
                ...
                if parts[2] == "hcpcs":
                    return self._send(
                        200, lookups["/v1/lookup/hcpcs/{code}"](key))
                if parts[2] == "hcpcs-section":
                    return self._send(
                        200, lookups["/v1/lookup/hcpcs-section/{section}"](
                            key, limit))
                if parts[2] == "hcpcs-search":
                    limit = qs.get("limit", ["50"])[0]
                    return self._send(
                        200, lookups["/v1/lookup/hcpcs-search/{term}"](
                            key, limit))
...
def make_server(store: HcpcsStore, *, host: str = "127.0.0.1", port: int = 0
...
def serve(db_path: str, *, host: str = "127.0.0.1", port: int = 8099) -> None:  # pragma: no cover
    ...
    store = HcpcsStore(db_path)
    server, bound = make_server(store, host=host, port=port)
    print(f"HCPCS /v1 surface on http://{host}:{bound}  (Ctrl-C to stop)")
    try:
