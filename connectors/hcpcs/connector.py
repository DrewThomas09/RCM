"""The HCPCS connector: ``discover()`` + ``fetch()``.
...
**code-prefix seeds** (``q=code:A*`` … ``code:V*``) declared on the
:class:`EndpointSpec`; each seed's result window is small enough to page
by ``offset`` under the ceiling — for HCPCS Level II (~8k codes) every
letter seed fits comfortably.
