"""Defensive accessors for data.healthcare.gov records.

DKAN datastore rows are flat dicts, but the metastore catalog items are
nested DCAT documents (``distribution`` is a list of objects,
``contactPoint``/``publisher`` are objects, ``keyword``/``theme`` are
lists that are sometimes absent). Two rules everywhere downstream:
