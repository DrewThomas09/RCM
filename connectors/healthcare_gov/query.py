...
sees the data.healthcare.gov datastore's native paging — that was absorbed at ingest
time (:mod:`connectors.healthcare_gov.connector`). Here the data is already
in canonical SQLite tables, so a query is plain parameterised SQL over a
registry-resolved table + ``source_endpoint`` slice.
...
from .tables import TABLES, HealthcareGovStore
...
def query(
    ...
    *,
    ...
    """Run a uniform query against a registered CMS Coverage dataset."""
