"""A standalone stdlib HTTP surface for the BLS QCEW slice.
...
both registered QCEW datasets and the two lookup handlers. If/when a
...
Routes (all GET, JSON out):
    ...
    /v1/lookup/labor-market/{area_fips}[?year=&qtr=&limit=N]
    /v1/lookup/industry-employment/{naics}[?year=&qtr=&limit=N]

Querystring → uniform query: reserved keys (`select`, `sort`, `limit`,
`offset`, `group_by`) are pulled out; every other key is a filter,
supporting the `field__op` grammar (`avg_wkly_wage__gte=2000`). The
caller never sees the slice-download mechanics — those were absorbed at
ingest.
"""
...
_CONNECTOR_NAME = "bls_qcew"
_CONNECTOR_LABEL = "BLS QCEW (healthcare employment + wages)"


def connector_status(store: BlsQcewStore) -> Dict[str, Any]:
...
def build_handler(store: BlsQcewStore):
    ...
    class BlsQcewHandler(BaseHTTPRequestHandler):
        server_version = "bls-qcew-connector/1.0"
        ...
        def _route(self, parts: List[str], qs: Dict[str, List[str]]) -> None:
            ...
            if len(parts) >= 3 and parts[0] == "v1" and parts[1] == "query":
                ...
                if len(parts) == 3:
                    qres: QueryResult = query(
                        store, dataset, filters=p["filters"],
            ...
            if len(parts) == 4 and parts[0] == "v1" and parts[1] == "lookup":
                ...
                qtr = qs.get("qtr", [None])[0]
                limit = qs.get("limit", ["25"])[0]
                if parts[2] == "labor-market":
                    return self._send(
                        200,
                        lookups["/v1/lookup/labor-market/{area_fips}"](
                            key, year, qtr, limit))
                if parts[2] == "industry-employment":
                    return self._send(
                        200,
                        lookups["/v1/lookup/industry-employment/{naics}"](
                            key, year, qtr, limit))
            self._send(404, {"error": f"no route for /{'/'.join(parts)}"})

    return BlsQcewHandler


def make_server(store: BlsQcewStore, *, host: str = "127.0.0.1", port: int = 0
...
def serve(db_path: str, *, host: str = "127.0.0.1",
          port: int = 8099) -> None:  # pragma: no cover
    """Open the store and serve forever (CLI entry)."""
    store = BlsQcewStore(db_path)
    server, bound = make_server(store, host=host, port=port)
    print(f"BLS QCEW /v1 surface on http://{host}:{bound}  (Ctrl-C to stop)")
    try:
