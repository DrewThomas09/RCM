...
sees the QCEW slice-download mechanics — the CSV pull was absorbed at
ingest time (:mod:`connectors.bls_qcew.connector`). Here the data is
already in the canonical SQLite table, so a query is plain
parameterised SQL over a registry-resolved table + ``source_endpoint``
slice (the two slice datasets share ``qcew_industry_area`` and are
pinned apart by it).
...
def query(
    ...
    *,
    ...
    """Run a uniform query against a registered QCEW dataset."""
    ...
    total = store.count(table,
                        where_sql.replace("WHERE ", "", 1) if where_sql else "",
                        args)
...
def aggregate(
    ...
    *,
    ...
    Served straight from the already-ingested canonical table (no extra
    slice round-trips): group by one or more whitelisted columns and count
