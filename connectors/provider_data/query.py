...
sees the DKAN datastore's native paging — that was absorbed at ingest
time (:mod:`connectors.provider_data.connector`). Here the data is
already in canonical SQLite tables, so a query is plain parameterised
SQL over a registry-resolved table + ``source_endpoint`` slice.

(Engine copied wholesale from ``connectors.cms_coverage.query`` — the
estate contract — with only the registry/tables imports, store type and
this docstring changed.)
...
def query(
    ...
    *,
    ...
    """Run a uniform query against a registered Provider Data dataset."""
