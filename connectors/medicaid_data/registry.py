"""Declarative dataset registry — one row per data.medicaid.gov dataset.
...
(:mod:`connectors.medicaid_data.query`) auto-exposes anything in this
registry.

Because the per-year NADAC and SDUD files land in one canonical table
each (``medicaid_nadac`` / ``medicaid_sdud`` — the estate's documented
shared-table pattern), each registry row carries a ``source_filter``
(the ``source_endpoint`` column value) so a dataset query returns exactly
that year's slice while still sharing one physical table. The generic
``fetched_rows`` dataset deliberately has an EMPTY ``source_filter``:
its table multiplexes arbitrary catalog datasets, and pinning to a fixed
value would hide them all — callers slice with a ``dataset_key`` filter
instead.

All rows are tagged ``source='medicaid_data'`` so the central aggregator
...
def _row(spec: EndpointSpec) -> RegistryRow:
    return RegistryRow(
        ...
        # Generic rows multiplex many dataset_keys in one table — no pin.
...
def registry_rows() -> List[RegistryRow]:
    """Every data.medicaid.gov dataset as a declarative registry row."""
