"""A standalone stdlib HTTP surface for the data.medicaid.gov slice.
...
every registered medicaid_data dataset and the three lookup handlers.
...
supporting the `field__op` grammar (`as_of_date__gte=2026-01-01`).
The caller never sees DKAN's native paging — it was absorbed at ingest.
"""
...
_CONNECTOR_NAME = "medicaid_data"
_CONNECTOR_LABEL = "Medicaid Open Data (data.medicaid.gov)"


def connector_status(store: MedicaidDataStore) -> Dict[str, Any]:
...
def build_handler(store: MedicaidDataStore):
    ...
    class MedicaidDataHandler(BaseHTTPRequestHandler):
        server_version = "medicaid-data-connector/1.0"
        ...
        def _route(self, parts: List[str], qs: Dict[str, List[str]]) -> None:
            ...
            if len(parts) == 4 and parts[0] == "v1" and parts[1] == "lookup":
                ...
                if parts[2] == "ndc-cost":
                    return self._send(
                        200, lookups["/v1/lookup/ndc-cost/{ndc}"](key))
                if parts[2] == "state-drug":
                    return self._send(
                        200, lookups["/v1/lookup/state-drug/{state}"](key))
                if parts[2] == "medicaid-dataset":
                    return self._send(
                        200,
                        lookups["/v1/lookup/medicaid-dataset/{identifier}"](key))
            self._send(404, {"error": f"no route for /{'/'.join(parts)}"})

    return MedicaidDataHandler


def make_server(store: MedicaidDataStore, *, host: str = "127.0.0.1",
...
def serve(db_path: str, *, host: str = "127.0.0.1", port: int = 8099) -> None:  # pragma: no cover
    ...
    store = MedicaidDataStore(db_path)
    server, bound = make_server(store, host=host, port=port)
    print(f"medicaid_data /v1 surface on http://{host}:{bound}  (Ctrl-C to stop)")
    try:
