"""Defensive accessors for data.medicaid.gov records.

DKAN datastore rows are flat dicts, but the metastore catalog nests
(``publisher.name``, ``contactPoint.hasEmail``, ``distribution[0]``) and
many DCAT fields are simply absent (``theme`` on 403/541 items,
``accrualPeriodicity`` on 273, ``issued`` on 95 — measured live
2026-07-06). Two rules everywhere downstream:
  ...
  2. Never assume cardinality — :func:`first`/:func:`as_list` collapse
     the scalar-or-list ambiguity; :func:`join_list` renders list fields
     (themes, keywords) as stable delimited TEXT for the canonical tables.
"""
