...
sees DKAN's native ``limit``/``offset`` paging — that was absorbed at
ingest time (:mod:`connectors.medicaid_data.connector`). Here the data
is already in canonical SQLite tables, so a query is plain parameterised
SQL over a registry-resolved table + ``source_endpoint`` slice (which is
what keeps the per-year NADAC/SDUD datasets separately queryable while
sharing one physical table).
...
def query(
    ...
    *,
    ...
    """Run a uniform query against a registered medicaid_data dataset."""
