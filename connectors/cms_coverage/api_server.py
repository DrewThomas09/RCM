"""A standalone stdlib HTTP surface for the CMS Coverage slice.
...
every registered CMS Coverage dataset and the two lookup handlers. If/when
...
supporting the `field__op` grammar (`last_updated_sort__gte=20240101`).
The caller never sees the API's native paging — it was absorbed at ingest.
"""
...
from .tables import TABLES, CmsCoverageStore
...
_CONNECTOR_NAME = "cms_coverage"
_CONNECTOR_LABEL = "CMS Medicare Coverage Database"


def connector_status(store: CmsCoverageStore) -> Dict[str, Any]:
...
def build_handler(store: CmsCoverageStore):
    ...
    class CmsCoverageHandler(BaseHTTPRequestHandler):
        server_version = "cms-coverage-connector/1.0"
        ...
        def _route(self, parts: List[str], qs: Dict[str, List[str]]) -> None:
            ...
            # /v1/lookup/document/{id} , /v1/lookup/contractor/{id}
            if len(parts) == 4 and parts[0] == "v1" and parts[1] == "lookup":
                key = parts[3]
                if parts[2] == "document":
                    return self._send(
                        200, lookups["/v1/lookup/document/{document_id}"](key))
                if parts[2] == "contractor":
                    return self._send(
                        200, lookups["/v1/lookup/contractor/{contractor_id}"](key))
            self._send(404, {"error": f"no route for /{'/'.join(parts)}"})

    return CmsCoverageHandler


def make_server(store: CmsCoverageStore, *, host: str = "127.0.0.1", port: int = 0
...
def serve(db_path: str, *, host: str = "127.0.0.1", port: int = 8098) -> None:  # pragma: no cover
    ...
    store = CmsCoverageStore(db_path)
    server, bound = make_server(store, host=host, port=port)
    print(f"CMS Coverage /v1 surface on http://{host}:{bound}  (Ctrl-C to stop)")
    try:
