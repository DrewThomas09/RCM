"""A standalone stdlib HTTP surface for the openFDA slice.
...
every registered openFDA dataset and the two lookup handlers. If/when a
...
Routes (all GET, JSON out):
    ...
    /v1/lookup/drug/{ndc}
    /v1/lookup/device/{product_code}

Querystring → uniform query: reserved keys (`select`, `sort`, `limit`,
`offset`) are pulled out; every other key is a filter, supporting the
`field__op` grammar (`receivedate__gte=20200101`). The caller never sees
openFDA's native paging — it was absorbed at ingest.
"""
...
from .tables import TABLES, OpenFdaStore
...
_CONNECTOR_NAME = "openfda"
_CONNECTOR_LABEL = "openFDA (drug + device)"


def connector_status(store: OpenFdaStore) -> Dict[str, Any]:
...
def build_handler(store: OpenFdaStore):
    ...
    class OpenFdaHandler(BaseHTTPRequestHandler):
        server_version = "openfda-connector/1.0"
        ...
        def _route(self, parts: List[str], qs: Dict[str, List[str]]) -> None:
            ...
            if parts == ["v1", "companies"]:
                q = qs.get("q", [""])[0]
                # Raw string through: search_companies clamps (junk limit
                # degrades to the default, never a ValueError → 500).
            ...
            # /v1/lookup/drug/{ndc} , /v1/lookup/device/{product_code}
            if len(parts) == 4 and parts[0] == "v1" and parts[1] == "lookup":
                key = parts[3]
                if parts[2] == "drug":
                    return self._send(200, lookups["/v1/lookup/drug/{ndc}"](key))
                if parts[2] == "device":
                    return self._send(
...
def make_server(store: OpenFdaStore, *, host: str = "127.0.0.1", port: int = 0
...
def serve(db_path: str, *, host: str = "127.0.0.1", port: int = 8099) -> None:  # pragma: no cover
    ...
    store = OpenFdaStore(db_path)
    server, bound = make_server(store, host=host, port=port)
    print(f"openFDA /v1 surface on http://{host}:{bound}  (Ctrl-C to stop)")
    try:
