...
sees Socrata's native paging — that was absorbed at ingest time
(:mod:`connectors.healthdata_gov.connector`). Here the data is already
in canonical SQLite tables, so a query is plain parameterised SQL over a
registry-resolved table + ``source_endpoint`` slice. (Copied wholesale
from ``connectors.cms_coverage.query`` per the estate contract; only the
imports, store type and docstrings differ.)
...
def query(
    ...
    *,
    ...
    """Run a uniform query against a registered healthdata_gov dataset."""
...
def aggregate(
    ...
    *,
    ...
    API round-trips): group by one or more whitelisted columns and count
