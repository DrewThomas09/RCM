"""Declarative dataset registry — one row per healthdata_gov dataset.
...
(:mod:`connectors.healthdata_gov.query`) auto-exposes anything in this
registry.
...
All rows are tagged ``source='healthdata_gov'`` so the central
...
def registry_rows() -> List[RegistryRow]:
    """Every healthdata_gov dataset as a declarative registry row."""
