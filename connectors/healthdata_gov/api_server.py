"""A standalone stdlib HTTP surface for the healthdata_gov slice.
...
every registered healthdata_gov dataset and the two lookup handlers.
...
supporting the `field__op` grammar (`date__gte=2021-01-01`). The caller
never sees Socrata's native paging — it was absorbed at ingest.
"""
...
_CONNECTOR_NAME = "healthdata_gov"
_CONNECTOR_LABEL = "healthdata.gov (HHS-wide meta-catalog)"


def connector_status(store: HealthdataGovStore) -> Dict[str, Any]:
...
def build_handler(store: HealthdataGovStore):
    ...
    class HealthdataGovHandler(BaseHTTPRequestHandler):
        server_version = "healthdata-gov-connector/1.0"
        ...
        def _route(self, parts: List[str], qs: Dict[str, List[str]]) -> None:
            ...
            # /v1/lookup/hhs-dataset/{uid} , /v1/lookup/hospital-capacity/{ccn}
            if len(parts) == 4 and parts[0] == "v1" and parts[1] == "lookup":
                key = parts[3]
                if parts[2] == "hhs-dataset":
                    return self._send(
                        200, lookups["/v1/lookup/hhs-dataset/{dataset_uid}"](key))
                if parts[2] == "hospital-capacity":
                    return self._send(
                        200, lookups["/v1/lookup/hospital-capacity/{ccn}"](key))
            self._send(404, {"error": f"no route for /{'/'.join(parts)}"})

    return HealthdataGovHandler


def make_server(store: HealthdataGovStore, *, host: str = "127.0.0.1",
...
def serve(db_path: str, *, host: str = "127.0.0.1", port: int = 8104) -> None:  # pragma: no cover
    """Open the store and serve forever (CLI entry)."""
    store = HealthdataGovStore(db_path)
    server, bound = make_server(store, host=host, port=port)
    print(f"healthdata_gov /v1 surface on http://{host}:{bound}  (Ctrl-C to stop)")
    try:
