"""HTTP transport for healthdata.gov (Socrata SODA): throttle + backoff.

healthdata.gov is the HHS-wide open-data meta-catalog, served by the same
Socrata platform as data.cdc.gov, and it works unauthenticated for modest
volume; unauthenticated clients share a throttled pool, so an optional
application token from ``$HEALTHDATA_APP_TOKEN`` is sent as the
``X-App-Token`` header when set (register one at
https://healthdata.gov/profile/edit/developer_settings). Mirroring the
estate's cdc_data finding on the same platform: a *wrong* token hard-403s,
so the header is only attached when the env var (or constructor) actually
provides one — never a placeholder.
...
USER_AGENT = "rcm-connectors/healthdata_gov"
DEFAULT_BASE_URL = "https://healthdata.gov"
...
class HealthdataGovApiError(RuntimeError):
    """Raised when healthdata.gov is unreachable or returns an unrecoverable error."""
...
@dataclass
class HealthdataSodaTransport:
    """Throttled, retrying JSON transport against ``healthdata.gov``.

    Timeout is higher than the estate's usual 30s: the catalog metadata
    endpoint serves 10k+ item pages and was measured at ~40-65s for a
    5000-item page live (2026-07-06), so a bulk ``discover`` needs the
    headroom. Row endpoints answer well inside it.
    """
    ...
    @classmethod
    def from_env(cls, **overrides: Any) -> "HealthdataSodaTransport":
        """Build a transport, reading an optional token from
        ``$HEALTHDATA_APP_TOKEN``.

        Absent token is fine — healthdata.gov serves unauthenticated
        requests from a shared throttled pool; the token only buys a
        dedicated rate allocation.
        """
        token = os.environ.get("HEALTHDATA_APP_TOKEN") or None
    ...
    def _headers(self) -> Dict[str, str]:
        ...
        # 403s on an invalid token, so a placeholder would be strictly
        # worse than anonymous access.
        if self.app_token:
    ...
    def get_json(
        ...
        *,
        ...
        :class:`HealthdataGovApiError` only after exhausting retries or
        on an unrecoverable 4xx.
        """
        ...
        for attempt in range(self.max_retries + 1):
            ...
            if status == 429 or status >= 500 or status == 0:
                ...
                if attempt < self.max_retries:
                    ...
                    continue
                break
            # Other 4xx won't fix on retry: 400 = bad SoQL; 403 = bad
            # token, OR (verified live) a catalog entry that serves no
            # rows on this domain — state-portal copies and href mirrors
            # of other portals all 403 on /resource/.
            hint = ("" if status != 403 else
                    " (403 here usually means the 4x4 is a federated/"
                    "mirror catalog entry that serves no rows on "
                    "healthdata.gov, or an invalid app token)")
            raise HealthdataGovApiError(
                f"{url}: HTTP {status} {_short_body(resp.body)}{hint}"
            )
        raise HealthdataGovApiError(
