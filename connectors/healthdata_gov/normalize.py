"""Map raw healthdata.gov payloads → canonical rows.
...
Cross-cutting normalizations done here:
  * Catalog metadata arrives camelCase (``dataUpdatedAt``); columns are
    ...
    ``customFields."Common Core"`` block. Unlike the cdc_data catalog
    mapper this one ALSO maps ``domain``: healthdata.gov is a
    meta-catalog whose entries split between HHS's own hub records
    (``domain=datahub.hhs.gov`` — the class containing every
    row-serving native dataset, plus href mirrors whose ``attribution``
    names the home portal, e.g. data.cdc.gov) and copies federated in
    from state/city portals (``domain=healthdata.gov``) — VERIFIED LIVE
    2026-07-06; the distinction is what makes mirror dedup against the
    rest of the estate possible.
...
# else (approvals, externalId, hideFromDataJson, …) is deliberately
# unmapped noise.
_CATALOG_KNOWN = {
    "id", "name", "description", "category", "attribution", "provenance",
    "domain", "createdAt", "dataUpdatedAt", "metadataUpdatedAt", "updatedAt",
...
def _catalog_row(rec: Dict[str, Any], res: NormalizeResult,
    ...
    if uid in (None, ""):
        return
    res.add("healthdata_gov_catalog", {
        "dataset_uid": uid,
        ...
        # attribution names the HOME portal of mirror entries
        # (data.cdc.gov, data.medicaid.gov, OpenPaymentsData.cms.gov …).
        "attribution": coalesce(rec, ["attribution"]),
        "provenance": coalesce(rec, ["provenance"]),
        # datahub.hhs.gov (HHS's own hub, incl. all row-serving natives)
        # vs healthdata.gov (state/city-portal copies) — verified live.
        "domain": coalesce(rec, ["domain"]),
...
def _curated_row(rec: Dict[str, Any], res: NormalizeResult,
    ...
    # normalizer (snake_case + SQL-safe rename). The healthdata.gov
    # curated snapshots contain no reserved words today, but the rule
    # stays mechanical so drift can never produce an unquotable column.
...
def _canonical_slice_params(slice_params: Optional[Dict[str, Any]]
    ...
    full-crawl case keeps its keys byte-identical run over run.
    """
    if not slice_params:
        return None
...
def slice_signature(slice_params: Optional[Dict[str, Any]] = None) -> str:
    ...
    ``""`` for an unfiltered fetch (the common case — keys stay
    ``{dataset_key}:{row_idx}``). Otherwise the first
    :data:`_SLICE_SIG_LEN` hex chars of the sha1 over the canonicalized
    (sorted-keys JSON, values coerced via ``str``) params. The signature
    becomes a key segment (``{dataset_key}:{sig}:{row_idx}``) so two
    fetches of the same dataset with different ``$where``/column filters
    can never overwrite each other's rows in the shared table —
    ``row_idx`` is only meaningful *within* one filter slice.
    """
...
def normalize_generic(dataset_key: str, raw_rows: List[Dict[str, Any]],
    ...
    """Raw rows from an arbitrary 4x4 → ``healthdata_gov_rows`` records.
    ...
    fetches keep the plain two-segment key shape.
    """
