"""The healthdata_gov connector: ``discover()`` + ``fetch()`` + ``refresh()``.
...
* The catalog metadata API (``/api/views/metadata/v1``) also returns a
  ...
  healthdata.gov (identical to the estate's data.cdc.gov finding) and
...
:data:`DEFAULT_MAX_PAGES` = 5) so an accidental "give me the facility
capacity file" doesn't hammer the API with a ~1M-row pull; pass a larger
``max_pages`` explicitly (bounded by :data:`MAX_PAGES_HARD_CAP`) for a
deliberate bulk run. ``discover`` uses its own default sized to drain
the whole catalog (23,080 items live on 2026-07-06 / 1000-per-page ≈ 24
pages).
"""
...
CATALOG_MAX_PAGES = 40         # catalog is ~24 pages at 1000/page; headroom
MAX_PAGES_HARD_CAP = 3000      # absolute bound: no loop can run away
...
class HealthdataGovConnector:
    ...
    def __init__(
        self,
        transport: Optional[HealthdataSodaTransport] = None,
        *,
        sleep: Callable[[float], None] = __import__("time").sleep,
    ) -> None:
        self.transport = transport or HealthdataSodaTransport.from_env()
        self._sleep = sleep

    # ── discovery: sync the full healthdata.gov catalog ───────────────
    def discover(
        self,
        *,
        ...
        healthdata.gov — native assets AND federated pointers — lands as
        one ``healthdata_gov_catalog`` row (23,080 live on 2026-07-06).
        When a ``store`` is passed the rows are also upserted (idempotent
        on the 4x4 id), making ``discover(store=...)`` the one-call
        catalog refresh.
        """
        ...
        for page in range(1, cap + 1):
            ...
            if len(items) < limit:      # short/empty page → catalog drained
                break
        rows = normalize(spec, raw).rows.get("healthdata_gov_catalog", [])
        if store is not None:
            store.upsert("healthdata_gov_catalog", rows)
        return rows
    ...
    def fetch(
        ...
        *,
        ...
        ``dataset`` is an endpoint key (``"hospital_ids"``), a full
        dataset id (``"healthdata_gov_hospital_ids"``), or an
    ...
    def fetch_dataset(
        ...
        *,
        ...
        """Fetch raw rows from ANY healthdata.gov 4x4 dataset on demand.

        The escape hatch that keeps the whole catalog reachable: rows go
        through :func:`normalize_generic` into ``healthdata_gov_rows``
        (see :meth:`refresh`), so even unregistered datasets stay
        queryable through the uniform engine. An unknown 4x4 yields zero
        rows (the API 404 folds to empty). A *mirror/federated* catalog
        entry — whose rows live on its home portal, not here — 403s
        live, which surfaces as a clear
        :class:`~connectors.healthdata_gov.transport.HealthdataGovApiError`
        with a hint rather than silently-empty data.
        """
    ...
    def _fetch_resource(
        ...
        *,
        ...
        # documented system ordering (verified live on this domain).
    ...
    def refresh(
        ...
        *,
        ...
        ``dataset`` may be an endpoint key, a full ``healthdata_gov_*``
        dataset id, or a raw 4x4 id (which lands in the generic
        ``healthdata_gov_rows`` table). Returns
        ...
        if spec is not None and spec.kind == "catalog":
            ...
            return {"dataset": spec.dataset_id, "endpoint": spec.key,
                    ...
                    "written": {"healthdata_gov_catalog": len(rows)}}
...
def _resolve(name: str) -> EndpointSpec:
    ...
    if spec is None:
        raise KeyError(
            f"unknown healthdata_gov dataset {name!r}; known keys: "
            f"{sorted(ENDPOINTS)} (or pass a raw 4x4 to fetch_dataset)")
    return spec


def _try_resolve(name: str) -> Optional[EndpointSpec]:
    """Accept a bare endpoint key or the estate-wide ``healthdata_gov_*`` id."""
    key = name[len("healthdata_gov_"):] if name.startswith("healthdata_gov_") else name
