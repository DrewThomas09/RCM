"""A standalone stdlib HTTP surface for the NPI Registry slice.

Demonstrates the API contract concretely **without touching any central
`/v1` router core**: this is a self-contained `http.server` app (the same
stdlib server the rest of RCM uses — no Flask/FastAPI) that mounts every
registered NPI dataset plus the lookup and validation handlers. If/when a
central pluggable router exists, the same registry + `v1_handlers` map
...
Routes (all GET, JSON out):
    ...
    /v1/lookup/taxonomy/{code}
    /v1/validate/npi/{npi}

Querystring → uniform query: reserved keys (`select`, `sort`, `limit`,
`offset`, `group_by`) are pulled out; every other key is a filter,
supporting the `field__op` grammar (`state__in=MD,CA`). The caller never
sees NPPES's native paging — it was absorbed at ingest.
"""
...
from .tables import TABLES, NpiStore
...
_CONNECTOR_NAME = "npi_registry"
_CONNECTOR_LABEL = "NPI Registry (NPPES v2.1)"


def connector_status(store: NpiStore) -> Dict[str, Any]:
...
def build_handler(store: NpiStore):
    ...
    class NpiHandler(BaseHTTPRequestHandler):
        server_version = "npi-registry-connector/1.0"
        ...
        def _route(self, parts: List[str], qs: Dict[str, List[str]]) -> None:
            ...
            # /v1/lookup/provider/{npi} , /v1/lookup/taxonomy/{code}
            if len(parts) == 4 and parts[0] == "v1" and parts[1] == "lookup":
                key = parts[3]
                if parts[2] == "provider":
                    return self._send(
                        200, lookups["/v1/lookup/provider/{npi}"](key))
                if parts[2] == "taxonomy":
                    return self._send(
                        200, lookups["/v1/lookup/taxonomy/{code}"](key))
            # /v1/validate/npi/{npi}
            if (len(parts) == 4 and parts[0] == "v1" and parts[1] == "validate"
                    and parts[2] == "npi"):
                return self._send(
                    200, lookups["/v1/validate/npi/{npi}"](parts[3]))
            self._send(404, {"error": f"no route for /{'/'.join(parts)}"})

    return NpiHandler


def make_server(store: NpiStore, *, host: str = "127.0.0.1", port: int = 0
...
def serve(db_path: str, *, host: str = "127.0.0.1", port: int = 8098) -> None:  # pragma: no cover
    """Open the store and serve forever (CLI entry)."""
    store = NpiStore(db_path)
    server, bound = make_server(store, host=host, port=port)
    print(f"NPI Registry /v1 surface on http://{host}:{bound}  (Ctrl-C to stop)")
    try:
