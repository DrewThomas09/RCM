...
sees NPPES's native paging — that was absorbed at ingest time
(:mod:`connectors.npi_registry.connector`). Here the data is already in
canonical SQLite tables, so a query is plain parameterised SQL over a
registry-resolved table.
...
clamped. Mirrors the RCM rules (parameterised SQL only, ``_clamp_int``
every integer param).
...
def query(
    ...
    *,
    ...
    """Run a uniform query against a registered NPI dataset."""
...
def _build_where(row: RegistryRow, filters: Dict[str, Any], colset: set
    ...
    clauses: List[str] = []
    args: List[Any] = []
    # Pin to this dataset's endpoint slice when the table is shared.
...
def aggregate(
    ...
    *,
    ...
    The cheap market-map path, served from the already-ingested canonical
    tables (no extra NPPES round-trips): group by one or more whitelisted
    columns and count rows, ordered by frequency. Same identifier
