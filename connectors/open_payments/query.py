...
sees DKAN's native ``limit``/``offset``/``conditions[...]`` grammar —
that was absorbed at ingest time
(:mod:`connectors.open_payments.connector`). Here the data is already
in canonical SQLite tables, so a query is plain parameterised SQL over a
registry-resolved table + ``source_endpoint`` slice (the generic
``open_payments_rows`` table carries an empty ``source_filter`` and is
sliced by a ``dataset_key`` filter instead).
...
def query(
    ...
    *,
    ...
    """Run a uniform query against a registered Open Payments dataset."""
