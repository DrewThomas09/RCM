"""A standalone stdlib HTTP surface for the Open Payments slice.
...
every registered Open Payments dataset and the three lookup handlers.
...
supporting the `field__op` grammar (`recipient_state=VT`,
`total_amount_of_payment_usdollars__gte=1000`). The caller never sees
DKAN's native paging — it was absorbed at ingest.
"""
...
def build_handler(store: OpenPaymentsStore):
    ...
    class OpenPaymentsHandler(BaseHTTPRequestHandler):
        server_version = "open-payments-connector/1.0"
        ...
        def _route(self, parts: List[str], qs: Dict[str, List[str]]) -> None:
            ...
            # /v1/lookup/{noun}/{key}, optional ?limit= for the samples.
            if len(parts) == 4 and parts[0] == "v1" and parts[1] == "lookup":
                key = parts[3]
                limit = qs.get("limit", [None])[0]
                if parts[2] == "physician-payments":
                    fn = lookups["/v1/lookup/physician-payments/{npi}"]
                    return self._send(
                        200, fn(key, limit) if limit is not None else fn(key))
                if parts[2] == "manufacturer":
                    fn = lookups["/v1/lookup/manufacturer/{name}"]
                    return self._send(
                        200, fn(key, limit) if limit is not None else fn(key))
                if parts[2] == "op-dataset":
                    return self._send(
                        200, lookups["/v1/lookup/op-dataset/{identifier}"](key))
            self._send(404, {"error": f"no route for /{'/'.join(parts)}"})

    return OpenPaymentsHandler


def make_server(store: OpenPaymentsStore, *, host: str = "127.0.0.1",
...
def serve(db_path: str, *, host: str = "127.0.0.1", port: int = 8099) -> None:  # pragma: no cover
    ...
    store = OpenPaymentsStore(db_path)
    server, bound = make_server(store, host=host, port=port)
    print(f"Open Payments /v1 surface on http://{host}:{bound}  (Ctrl-C to stop)")
    try:
