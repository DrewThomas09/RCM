"""Declarative dataset registry — one row per Open Payments dataset.
...
(:mod:`connectors.open_payments.query`) auto-exposes anything in this
registry.

``source_filter`` (the ``source_endpoint`` column value the query engine
pins on) is the spec key for the catalog and every curated table. The
generic ``open_payments_fetched_rows`` dataset deliberately carries an
EMPTY ``source_filter``: its shared table stores the *catalog UUID* in
``source_endpoint`` (one slice per on-demand dataset), so pinning to a
single literal would hide every row — callers slice with a
``dataset_key`` filter instead.

All rows are tagged ``source='open_payments'`` so the central aggregator
...
def _row(spec: EndpointSpec) -> RegistryRow:
    return RegistryRow(
        ...
        # Generic rows are sliced by dataset_key, not a fixed endpoint tag.
...
def registry_rows() -> List[RegistryRow]:
    """Every Open Payments dataset as a declarative registry row."""
