...
RawResponse``) that serves canned ICD-10 records per endpoint path and
...
Records are plain dicts (e.g. ``{"code": "E11.65", "name": "..."}``);
...
a record lacking ``long_name`` yields a narrower row and exercises the
...
class FakeNlm:
    ...
    def __call__(self, url: str, headers: Dict[str, str], timeout: float
        ...
        df = unquote_plus(qs.get("df", ["code,name"])[0]).split(",")
        total = len(recs)
