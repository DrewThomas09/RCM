"""A standalone stdlib HTTP surface for the ICD-10 slice.
...
both registered ICD-10 datasets and the lookup/search handlers. If/when
...
_RESERVED = {"select", "sort", "limit", "offset", "group_by", "metric", "type", "q"}
...
_CONNECTOR_NAME = "icd10"
_CONNECTOR_LABEL = "ICD-10-CM + ICD-10-PCS (NLM Clinical Tables)"


def connector_status(store: Icd10Store) -> Dict[str, Any]:
...
def build_handler(store: Icd10Store):
    ...
    class Icd10Handler(BaseHTTPRequestHandler):
        server_version = "icd10-connector/1.0"
        ...
        def _route(self, parts: List[str], qs: Dict[str, List[str]]) -> None:
            ...
            # /v1/search/{code_type}?q=<term>&limit=
            if len(parts) == 3 and parts[0] == "v1" and parts[1] == "search":
                code_type = parts[2]
                q = qs.get("q", [""])[0]
                limit = qs.get("limit", ["50"])[0]
                return self._send(
                    200, lookups["/v1/search/{code_type}"](code_type, q, limit))
            # /v1/lookup/code/{code} , /v1/lookup/category/{category}
            if len(parts) == 4 and parts[0] == "v1" and parts[1] == "lookup":
                key = parts[3]
                code_type = qs.get("type", ["cm"])[0]
                if parts[2] == "code":
                    return self._send(
                        200, lookups["/v1/lookup/code/{code}"](key, code_type))
                if parts[2] == "category":
                    return self._send(
                        200, lookups["/v1/lookup/category/{category}"](key, code_type))
            self._send(404, {"error": f"no route for /{'/'.join(parts)}"})

    return Icd10Handler


def make_server(store: Icd10Store, *, host: str = "127.0.0.1", port: int = 0
...
def serve(db_path: str, *, host: str = "127.0.0.1", port: int = 8098) -> None:  # pragma: no cover
    ...
    store = Icd10Store(db_path)
    server, bound = make_server(store, host=host, port=port)
    print(f"ICD-10 /v1 surface on http://{host}:{bound}  (Ctrl-C to stop)")
    try:
