"""The canonical ICD-10 table + an idempotent SQLite store.

A single normalization target for this workstream:

  dim_icd10_code  — one row per ICD-10 code (CM diagnosis or PCS
                    procedure), keyed by ``code_key`` = ``{code_type}:{code}``.
...
TABLES: Dict[str, TableDef] = {
    "dim_icd10_code": TableDef(
        "dim_icd10_code", "code_key",
        ("code_key", "code_type", "code", "name", "long_name", "chapter",
         "category", "billable", *_META),
    ),
}

CANONICAL_TABLES: Tuple[str, ...] = ("dim_icd10_code",)


class Icd10Store:
    """Thin SQLite wrapper: schema bootstrap + idempotent batch upsert.

    The only module that talks to the ICD-10 SQLite file directly,
    ...
    def ensure_schema(self) -> None:
        ...
        cur.execute("CREATE INDEX IF NOT EXISTS ix_icd10_type_code "
                    "ON dim_icd10_code(code_type, code)")
        cur.execute("CREATE INDEX IF NOT EXISTS ix_icd10_chapter "
                    "ON dim_icd10_code(chapter)")
        cur.execute("CREATE INDEX IF NOT EXISTS ix_icd10_category "
                    "ON dim_icd10_code(code_type, category)")
        self.conn.commit()
