"""A standalone stdlib HTTP surface for the HRSA data slice.
...
every registered HRSA dataset and the two lookup handlers. If/when a
...
Routes (all GET, JSON out):
    ...
    /v1/lookup/shortage-area/{state}[?limit=N]
    /v1/lookup/health-center/{state}[?limit=N]

Querystring → uniform query: reserved keys (`select`, `sort`, `limit`,
`offset`, `group_by`) are pulled out; every other key is a filter,
supporting the `field__op` grammar (`hpsa_score__gte=18`). The caller
never sees the CSV download mechanics — those were absorbed at ingest.
"""
...
_CONNECTOR_NAME = "hrsa_data"
_CONNECTOR_LABEL = "HRSA (HPSA shortage areas + health centers)"


def connector_status(store: HrsaDataStore) -> Dict[str, Any]:
...
def build_handler(store: HrsaDataStore):
    ...
    class HrsaDataHandler(BaseHTTPRequestHandler):
        server_version = "hrsa-data-connector/1.0"
        ...
        def _route(self, parts: List[str], qs: Dict[str, List[str]]) -> None:
            ...
            # /v1/lookup/shortage-area/{state} , /v1/lookup/health-center/{state}
            if len(parts) == 4 and parts[0] == "v1" and parts[1] == "lookup":
                key = parts[3]
                limit = qs.get("limit", ["25"])[0]
                if parts[2] == "shortage-area":
                    return self._send(
                        200,
                        lookups["/v1/lookup/shortage-area/{state}"](key, limit))
                if parts[2] == "health-center":
                    return self._send(
                        200,
                        lookups["/v1/lookup/health-center/{state}"](key, limit))
            self._send(404, {"error": f"no route for /{'/'.join(parts)}"})

    return HrsaDataHandler


def make_server(store: HrsaDataStore, *, host: str = "127.0.0.1", port: int = 0
...
def serve(db_path: str, *, host: str = "127.0.0.1", port: int = 8099) -> None:  # pragma: no cover
    ...
    store = HrsaDataStore(db_path)
    server, bound = make_server(store, host=host, port=port)
    print(f"HRSA data /v1 surface on http://{host}:{bound}  (Ctrl-C to stop)")
    try:
