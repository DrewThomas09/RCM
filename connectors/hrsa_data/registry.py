"""Declarative dataset registry — one row per HRSA download dataset.
...
(:mod:`connectors.hrsa_data.query`) auto-exposes anything in this
registry.

Because the three HPSA discipline files land in the same canonical
table (``hrsa_hpsa``), each registry row also carries a
``source_filter`` (the ``source_endpoint`` column value) so a dataset
query returns exactly that file's slice while still sharing one
physical table.

All rows are tagged ``source='hrsa_data'`` so the central aggregator
...
def registry_rows() -> List[RegistryRow]:
    """Every HRSA download dataset as a declarative registry row."""
