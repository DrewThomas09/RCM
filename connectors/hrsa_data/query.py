...
sees the HRSA download mechanics — the CSV pull was absorbed at ingest
time (:mod:`connectors.hrsa_data.connector`). Here the data is already
in canonical SQLite tables, so a query is plain parameterised SQL over a
registry-resolved table + ``source_endpoint`` slice (the three HPSA
discipline datasets share ``hrsa_hpsa`` and are pinned apart by it).
...
def query(
    ...
    *,
    ...
    """Run a uniform query against a registered HRSA dataset."""
