"""A standalone stdlib HTTP surface for the cdc_data slice.
...
every registered cdc_data dataset and the two lookup handlers. If/when
...
supporting the `field__op` grammar (`year__gte=2020`). The caller never
sees Socrata's native paging — it was absorbed at ingest.
"""
...
_CONNECTOR_NAME = "cdc_data"
_CONNECTOR_LABEL = "CDC Open Data (data.cdc.gov / Socrata)"


def connector_status(store: CdcDataStore) -> Dict[str, Any]:
...
def build_handler(store: CdcDataStore):
    ...
    class CdcDataHandler(BaseHTTPRequestHandler):
        server_version = "cdc-data-connector/1.0"
        ...
        def _route(self, parts: List[str], qs: Dict[str, List[str]]) -> None:
            ...
            # /v1/lookup/county-health/{fips} , /v1/lookup/cdc-dataset/{uid}
            if len(parts) == 4 and parts[0] == "v1" and parts[1] == "lookup":
                key = parts[3]
                if parts[2] == "county-health":
                    return self._send(
                        200, lookups["/v1/lookup/county-health/{fips}"](key))
                if parts[2] == "cdc-dataset":
                    return self._send(
                        200, lookups["/v1/lookup/cdc-dataset/{dataset_uid}"](key))
            self._send(404, {"error": f"no route for /{'/'.join(parts)}"})

    return CdcDataHandler


def make_server(store: CdcDataStore, *, host: str = "127.0.0.1", port: int = 0
...
def serve(db_path: str, *, host: str = "127.0.0.1", port: int = 8099) -> None:  # pragma: no cover
    ...
    store = CdcDataStore(db_path)
    server, bound = make_server(store, host=host, port=port)
    print(f"cdc_data /v1 surface on http://{host}:{bound}  (Ctrl-C to stop)")
    try:
