"""A standalone stdlib HTTP surface for the NIH RePORTER slice.
...
every registered NIH RePORTER dataset and the two lookup handlers. If/when
...
Routes (all GET, JSON out):
    ...
    /v1/lookup/grantee-org/{name}[?limit=N]

Querystring → uniform query: reserved keys (`select`, `sort`, `limit`,
`offset`, `group_by`) are pulled out; every other key is a filter,
supporting the `field__op` grammar (`fiscal_year__gte=2024`). The caller
never sees the API's native POST-body paging — it was absorbed at ingest.
"""
...
from .tables import TABLES, NihReporterStore
...
_CONNECTOR_LABEL = "NIH RePORTER (grants + publications)"


def connector_status(store: NihReporterStore) -> Dict[str, Any]:
...
def build_handler(store: NihReporterStore):
    ...
    class NihReporterHandler(BaseHTTPRequestHandler):
        server_version = "nih-reporter-connector/1.0"
        ...
        def _route(self, parts: List[str], qs: Dict[str, List[str]]) -> None:
            ...
            # /v1/lookup/grant/{project_num} , /v1/lookup/grantee-org/{name}
            if len(parts) == 4 and parts[0] == "v1" and parts[1] == "lookup":
                key = parts[3]
                if parts[2] == "grant":
                    return self._send(
                        200, lookups["/v1/lookup/grant/{project_num}"](key))
                if parts[2] == "grantee-org":
                    handler = lookups["/v1/lookup/grantee-org/{name}"]
                    limit = qs.get("limit", [None])[0]
                    if limit is not None:
                        return self._send(200, handler(key, limit))
                    return self._send(200, handler(key))
            self._send(404, {"error": f"no route for /{'/'.join(parts)}"})

    return NihReporterHandler


def make_server(store: NihReporterStore, *, host: str = "127.0.0.1", port: int = 0
...
def serve(db_path: str, *, host: str = "127.0.0.1", port: int = 8099) -> None:  # pragma: no cover
    ...
    store = NihReporterStore(db_path)
    server, bound = make_server(store, host=host, port=port)
    print(f"NIH RePORTER /v1 surface on http://{host}:{bound}  (Ctrl-C to stop)")
    try:
