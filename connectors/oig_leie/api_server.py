"""A standalone stdlib HTTP surface for the OIG LEIE slice.
...
every registered LEIE dataset and the two lookup handlers. If/when a
...
Routes (all GET, JSON out):
    ...
    /v1/lookup/exclusion/{npi}[?limit=N]
    /v1/lookup/exclusion-name/{name}[?first=F&limit=N]

Querystring → uniform query: reserved keys (`select`, `sort`, `limit`,
`offset`, `group_by`) are pulled out; every other key is a filter,
supporting the `field__op` grammar (`excldate__gte=2024-01-01`). The
caller never sees the CSV download mechanics — those were absorbed at
ingest.
"""
...
_CONNECTOR_NAME = "oig_leie"
_CONNECTOR_LABEL = "HHS OIG LEIE (exclusion list)"


def connector_status(store: OigLeieStore) -> Dict[str, Any]:
...
def build_handler(store: OigLeieStore):
    ...
    class OigLeieHandler(BaseHTTPRequestHandler):
        server_version = "oig-leie-connector/1.0"
        ...
        def _route(self, parts: List[str], qs: Dict[str, List[str]]) -> None:
            ...
            # /v1/lookup/exclusion/{npi} , /v1/lookup/exclusion-name/{name}
            if len(parts) == 4 and parts[0] == "v1" and parts[1] == "lookup":
                key = parts[3]
                limit = qs.get("limit", ["25"])[0]
                if parts[2] == "exclusion":
                    return self._send(
                        200,
                        lookups["/v1/lookup/exclusion/{npi}"](key, limit))
                if parts[2] == "exclusion-name":
                    first = qs.get("first", [""])[0]
                    return self._send(
                        200,
                        lookups["/v1/lookup/exclusion-name/{name}"](
                            key, first, limit))
            self._send(404, {"error": f"no route for /{'/'.join(parts)}"})

    return OigLeieHandler


def make_server(store: OigLeieStore, *, host: str = "127.0.0.1", port: int = 0
...
def serve(db_path: str, *, host: str = "127.0.0.1", port: int = 8099) -> None:  # pragma: no cover
    ...
    store = OigLeieStore(db_path)
    server, bound = make_server(store, host=host, port=port)
    print(f"OIG LEIE /v1 surface on http://{host}:{bound}  (Ctrl-C to stop)")
    try:
