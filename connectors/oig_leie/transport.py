"""HTTP transport for OIG LEIE downloads: streamed CSV + retry/backoff.

The HHS Office of Inspector General publishes the List of Excluded
Individuals/Entities as plain CSV files under
``https://oig.hhs.gov/exclusions/downloadables/`` — there is no JSON API
and no key. That shapes this transport in two ways:
  ...
  * **Streaming, not buffering** — the default opener hands the live
    ...
    reads it incrementally, so a ``max_rows``-capped ingest of the
    ~15 MB full file never buffers the whole body. Fake openers used in
    tests keep the simple ``body: bytes`` contract;
    :meth:`RawResponse.reader` unifies both.
...
Politeness: OIG publishes no rate limit for the download files; the
...
_DEFAULT_USER_AGENT = (
    "rcm/oig-leie-connector (github.com/DrewThomas09/RCM; "
...
class OigLeieApiError(RuntimeError):
    """Raised when OIG is unreachable or returns an unrecoverable error.

    ``status`` carries the final HTTP status when one exists (``404``
    lets the connector's supplement walk-back distinguish "this month
    published no file" from a real failure without string matching).
    """
...
def _default_opener(url: str, headers: Dict[str, str], timeout_s: float) -> RawResponse:
    ...
    ``max_rows`` without downloading the rest of a 15 MB file.
    """
...
class CsvResult:
    ...
    ``fieldnames`` are the *raw* header names as published (uppercase —
    ``LASTNAME`` … ``WVRSTATE``) with empty trailing header cells
    dropped defensively. ``rows`` are raw ``{header: value}`` dicts;
    snake_casing, date/NPI cleanup and key composition happen in
    :mod:`connectors.oig_leie.normalize`.
    """
...
@dataclass
class OigLeieTransport:
    """Throttled, retrying CSV transport against ``oig.hhs.gov``."""

    user_agent: str = _DEFAULT_USER_AGENT
    base_url: str = _OIG_BASE
    ...
    @classmethod
    def from_env(cls, **overrides: Any) -> "OigLeieTransport":
        """Build a transport. No key exists for LEIE downloads; env only
        tunes the user agent so a shared crawler can identify itself."""
        import os
        params: Dict[str, Any] = {}
        ua = os.environ.get("OIG_LEIE_USER_AGENT")
        if ua:
    ...
    def build_url(self, path: str, params: Optional[Dict[str, Any]] = None) -> str:
        """Pure URL builder. LEIE download files take no query params in
    ...
    def get_csv(
        ...
        *,
        ...
        :class:`OigLeieApiError` after exhausting retries, on an
        unrecoverable 4xx, or on a 404 (``status=404`` on the exception;
        for a supplement path it usually means that month published no
        file — see https://oig.hhs.gov/exclusions/exclusions_list.asp
        for the published index).
        """
        opener = opener or _default_opener
        url = self.build_url(path, params)
        # Accept stays */* for parity with the estate's other CSV
        # transports (data.hrsa.gov answers 406 to ``Accept: text/csv``;
        # oig.hhs.gov served ``*/*`` fine in the live probe 2026-07-06).
        ...
        for attempt in range(self.max_retries + 1):
            ...
            if status == 404:
                ...
                raise OigLeieApiError(
                    f"{url}: HTTP 404 — LEIE file not found. The full "
                    f"database is always at "
                    f"{self.base_url}/exclusions/downloadables/UPDATED.csv; "
                    f"monthly supplement files are published per month and "
                    f"some months have none (verified live: no 2505excl.csv"
                    f"). See the published index at "
                    f"https://oig.hhs.gov/exclusions/exclusions_list.asp",
                    status=404)
            ...
            resp.close()
            raise OigLeieApiError(f"{url}: HTTP {status} {detail}",
                                  status=status)
        raise OigLeieApiError(
            f"{url}: failed after {self.max_retries + 1} attempts "
            f"(last status {last_status}: {last_detail})",
            status=last_status if last_status > 0 else None,
        )
    ...
    def _parse_csv(resp: RawResponse, max_rows: Optional[int], url: str
        ...
        Robustness rules (the live LEIE files are clean, but these cost
        nothing and future-proof against the quirks the other estate CSV
        sources exhibit):

          * decoded as ``utf-8-sig`` so a BOM, if OIG ever adds one,
        ...
        if not keep:
            raise OigLeieApiError(f"{url}: header row has no named columns")
