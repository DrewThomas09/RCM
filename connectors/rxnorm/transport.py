"""HTTP transport for the NLM RxNav (RxNorm) REST API.

RxNav (``rxnav.nlm.nih.gov/REST``) is public and keyless. Every endpoint
this connector uses answers a top-level JSON **object** wrapped in a
per-endpoint envelope key (``idGroup`` / ``properties`` /
``relatedGroup`` / ``rxclassDrugInfoList`` / ``approximateGroup``), so —
unlike the ICD-10/HCPCS transport that drains NLM Clinical Tables arrays
— this one requires a top-level dict.

Empty-result note: RxNav answers an unresolvable identifier with HTTP
200 and an envelope missing its payload key (rather than a 404), so the
normalizer treats a missing key as "no match", not as an error.
...
_DEFAULT_USER_AGENT = (
    "rcm-mc/rxnorm-connector (github.com/DrewThomas09/RCM; "
...
class RxNavApiError(RuntimeError):
    """Raised when RxNav is unreachable or returns an unrecoverable error."""
...
@dataclass
class RxNavTransport:
    """Throttled, retrying JSON transport against the RxNav API."""

    api_key: Optional[str] = None
    user_agent: str = _DEFAULT_USER_AGENT
    base_url: str = _RXNAV_BASE
    ...
    @classmethod
    def from_env(cls, **overrides: Any) -> "RxNavTransport":
        """Build a transport. RxNav needs no key; ``$RXNAV_API_KEY`` is
        read only for parity with the rest of the estate and is harmless
        when absent."""
        key = os.environ.get("RXNAV_API_KEY") or None
    ...
    def get_json(
        ...
        *,
        ...
        Returns the decoded RxNav payload — a **JSON object** carrying a
        per-endpoint envelope key. A 404 returns ``{}`` (an unknown path
        or identifier is an empty result, not an error). Raises
        :class:`RxNavApiError` only after exhausting retries or on an
        unrecoverable 4xx.
        """
        ...
        for attempt in range(self.max_retries + 1):
            ...
            if status == 404:
                # Unknown identifier — empty result.
                return {}
    ...
    def _parse(body: bytes, url: str) -> Dict[str, Any]:
        ...
        if not isinstance(doc, dict):
            raise RxNavApiError(f"{url}: unexpected JSON shape {type(doc).__name__}")
        return doc
