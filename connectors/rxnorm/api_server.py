"""A standalone stdlib HTTP surface for the RxNorm slice.

Demonstrates the API contract concretely **without touching the `/v1`
router core**: a self-contained `http.server` app (the same stdlib
server the rest of RCM-MC uses — no Flask/FastAPI) that mounts the
registered RxNorm datasets and the lookup handlers. The same registry +
`v1_handlers` map drive the unified estate server; this standalone
surface proves the contract and is testable end to end.
...
supporting the `field__op` grammar (`rxcui__like=15%`). The caller
never sees RxNav's per-endpoint envelopes — they were absorbed at ingest.
"""
...
from .tables import TABLES, RxNormStore
...
_CONNECTOR_NAME = "rxnorm"
_CONNECTOR_LABEL = "RxNorm drug identity (NLM RxNav)"


def connector_status(store: RxNormStore) -> Dict[str, Any]:
...
def build_handler(store: RxNormStore):
    ...
    class RxNormHandler(BaseHTTPRequestHandler):
        server_version = "rxnorm-connector/1.0"
        ...
        def _route(self, parts: List[str], qs: Dict[str, List[str]]) -> None:
            ...
            # /v1/lookup/rxnorm-ndc/{ndc} , /v1/lookup/rxnorm-concept/{rxcui},
            # /v1/lookup/rxnorm-name/{name}
            if len(parts) == 4 and parts[0] == "v1" and parts[1] == "lookup":
                key = parts[3]
                if parts[2] == "rxnorm-ndc":
                    return self._send(
                        200, lookups["/v1/lookup/rxnorm-ndc/{ndc}"](key))
                if parts[2] == "rxnorm-concept":
                    return self._send(
                        200,
                        lookups["/v1/lookup/rxnorm-concept/{rxcui}"](key))
                if parts[2] == "rxnorm-name":
                    return self._send(
                        200, lookups["/v1/lookup/rxnorm-name/{name}"](key))
            self._send(404, {"error": f"no route for /{'/'.join(parts)}"})

    return RxNormHandler


def make_server(store: RxNormStore, *, host: str = "127.0.0.1", port: int = 0
...
def serve(db_path: str, *, host: str = "127.0.0.1", port: int = 8101) -> None:  # pragma: no cover
    """Open the store and serve forever (CLI entry)."""
    store = RxNormStore(db_path)
    server, bound = make_server(store, host=host, port=port)
    print(f"RxNorm /v1 surface on http://{host}:{bound}  (Ctrl-C to stop)")
    try:
