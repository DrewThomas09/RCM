"""Declarative dataset registry — one row per Census ACS dataset.
...
(:mod:`connectors.census_acs.query`) auto-exposes anything in this
registry.

Each profile owns its table outright (county/state/CBSA have different
natural keys), but every row still carries ``source_filter`` — the
``source_endpoint`` column value — so the shared query engine's
slice-pinning stays uniform with the connectors whose tables *are*
shared.

The ``endpoint`` field is the detail-dataset path template
(``/{year}/acs/acs5``); every profile additionally reads the subject
dataset (``/{year}/acs/acs5/subject``) — that second call is a connector
implementation detail, absorbed like paging is elsewhere.

All rows are tagged ``source='census_acs'`` so the central aggregator
...
def registry_rows() -> List[RegistryRow]:
    """Every Census ACS dataset as a declarative registry row."""
